...
Options:
  ...
  --variant VAR         caches (default) or no_caches
  --encoding ENC        coeff (default) or eval (WHIR leaf encoding)
...
# crate needs nothing extra — the generated code is self-contained.
  ...
  case "$step" in
    ...
    prover)
      run_step "Generate proof" \
        run_prover_cargo \
          --features gkr_self_checks "${VARIANT_FEATURES[@]+"${VARIANT_FEATURES[@]}"}" "${ENCODING_PROVER_FEATURES[@]+"${ENCODING_PROVER_FEATURES[@]}"}" \
    ...
    malicious)
      run_step "Generate malicious proofs (corrupt witness, no self-checks)" \
        run_prover_cargo \
          --no-default-features --features prover,bincode "${VARIANT_FEATURES[@]+"${VARIANT_FEATURES[@]}"}" "${ENCODING_PROVER_FEATURES[@]+"${ENCODING_PROVER_FEATURES[@]}"}" \
