...
- Producer-parity history: confirmed historical GKR layer-mapping defect in a pre-assembly prover
...
- Activation condition: completing proof assembly or exposing the affected forward/Sumcheck chain as a proof-producing entrypoint
...
At this revision `prove_configured_with_gkr` still ended in `todo!()`, so the
bad mapping could be exercised by component code but could not yet add a new
end-to-end proof rejection. If proof assembly had been completed unchanged, the
wrong first-round output access would have disconnected the Sumcheck relation
from the forward layer polynomial. If both prover and verifier later shared a
wrong but consistent permutation, soundness would depend on whether committed
inputs and outputs still pinned the intended circuit wiring.
