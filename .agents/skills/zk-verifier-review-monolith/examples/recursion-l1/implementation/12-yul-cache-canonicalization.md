# Yul cache writes were explicitly reduced modulo the field

## Classification

- Historical field-representation hardening; no demonstrated vulnerability
...
- Security character: cache words could be noncanonical, but every identified consumer preserved their field congruence
...
For example, a mathematical gate value `x` could be cached as `x + P`. In the
affected generated source, the identified cache consumers feed the word through
`mulmod` or add it into an accumulator whose final check reduces modulo `P`.
History does not identify the raw equality or overflow-sensitive consumer
hypothesized by the earlier card.
...
5. No historical accepting or rejecting divergence is established because the
   relevant operations preserve congruence modulo `P` and reachable values fit
   within the 256-bit intermediates examined.

The history establishes a representation inconsistency, not a vulnerability.
Without an exact non-modular consumer or overflowing intermediate, it belongs in
implementation hardening rather than the soundness/completeness corpus.

## Impact and fix

The direct fix stores `mod(gate, P)` at the cache boundary, making the local type
invariant explicit. The same change set also hardens generated
negative-coefficient terms and fills missing generated constraint cases; those
are separate changes and cannot be attributed to cache canonicalization.
