...
- Security character: 41 base evaluations were outside the randomized handoff batch
...
The EVM verifier did not bind/authenticate the complete layer-0 input vector and parsed the remainder under the wrong grammar. The fix updates the count to 113, adds a matching one-shot transcript function, advances by 113 encodings, and makes the generator assert `memory + witness + setup` width.
