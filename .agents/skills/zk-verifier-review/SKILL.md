...
description: Defensively audit Rust and Solidity/Yul zero-knowledge proof verifiers, verifier generators, and whole-system aggregation/recursion/L1 settlement layers for protocol-implementation soundness, especially Fiat-Shamir transcript, proof-input validation, batching, Sumcheck/GKR, WHIR/FRI/DEEP-ALI, polynomial commitments, grinding, implementation parity, cross-circuit or cross-chunk composition, recursive-chain anchoring, calldata parsing, and deployed-contract acceptance bugs. Use for authorized read-only focused reviews or prover-wide verifier audits of current Airbender GKR verifiers in verifier_common/, verifier_generator/, full_statement_verifier/, verifier_evm/, recursive/L1 implementations, historical Airbender STARK verifiers, or comparable proof systems; use the prover only as a protocol/format cross-reference, not as the primary audit target.
---
