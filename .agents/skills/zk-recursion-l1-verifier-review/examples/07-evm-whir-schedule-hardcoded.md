...
- Confirmed historical EVM proof/configuration implementation gap
...
- Security character: cross-configuration proof-language drift; generally honest rejection, potentially wrong-parameter acceptance when deployment identity is not pinned
...
Rust and EVM proof languages could drift across any schedule change. The fix introduces `WhirGenConfig`, validates schedule lengths/power-of-two/bounds, derives generators and all geometry, renders a per-round switch, and passes the same folds/queries to calldata flattening.
