# Latent: Yul Sumcheck failures were stored but never rejected
...
The vulnerable parent belongs to the prototype sequence and predates the first
passing build named by the fix commit itself: `4f8d993` calls the large-pointer
variant the “first passing build.” No compiled contract or successful proof
consumer is established for the dummy-check revision. The missing reverts are
exact and would be catastrophic on activation, but they do not show that a
historical runnable verifier accepted a proof.
...
6. If a contract containing this source is compiled and selected, complete the
   remaining parser/WHIR checks and reach success.
...
If compiled and selected, core Sumcheck and GKR terminal verification would be
unenforced. The fix replaces dummy stores with immediate
`if error { revert(0,0) }` checks and regenerates the complete point-gate
batches/order.
