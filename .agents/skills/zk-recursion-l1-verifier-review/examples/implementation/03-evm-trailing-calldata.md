# WHIR cursor hardening rejected trailing calldata

## Classification

- Historical parser-canonicalization hardening; not a demonstrated vulnerability
- Boundary: WHIR proof byte stream → registry success notification
- Component: final calldata cursor/exhaustion check
- Security character: multiple byte encodings for one already-valid proof; no false statement or honest-proof rejection established
...
This does not prove a false polynomial statement and does not reject a canonical
honest proof. History also does not identify a relayer, signature, proof-hash, or
concatenated-message consumer that assigned semantics to the ignored suffix.

## Impact and fix

The generated test contract accepted a prefix language rather than an exact proof
language. The fix requires `REG_CD == calldatasize()` after all rounds and reverts
before registry notification on any suffix. This is good boundary hardening, but
the historical evidence does not support soundness or completeness severity.
