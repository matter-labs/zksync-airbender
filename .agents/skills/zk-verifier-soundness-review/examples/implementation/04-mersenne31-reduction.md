...
- Implementation history: confirmed field-arithmetic correctness bug without a verifier-facing call path
...
- Reachability: the defective signed-reduction helper had no non-definition caller at the vulnerable revision, and observed `from_u64` callers supplied internal small constants; a verifier-facing large-input path was not established
...
The commit confirms arithmetic defects, but a scoped history search found no caller of `from_negative_u64_with_reduction` outside its definition at the vulnerable revision. The observed `from_u64` callers constructed internal circuit constants rather than decoding attacker-controlled 64-bit proof words. This is therefore a concrete library implementation defect, not a latent verifier defect or evidence of an active verifier bypass. A soundness report must trace a verifier-facing large-input call site before asserting challenge bias or proof malleability.

## Impact and fix

If activated on large inputs, signed intermediates would map to the wrong field element and noncanonical unsigned values could pass the constructor's pre-cast check. The fix performs complete Mersenne folding including bits 62–63 and compares the full `u64` against `ORDER` before casting.
