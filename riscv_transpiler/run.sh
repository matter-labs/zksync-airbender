...
# reference block:
#   opt1 (default)      : 4 RISC-V values in host GPRs, timestamps in the 4 freed GPRs
#   opt2 (ts_in_xmm)    : 8 RISC-V values in host GPRs, timestamps in XMM lanes
#
...
echo "Building opt1 (default: 4 GPRs + GPR timestamps)..." >&2
B1="$(build_bin "jit")"            || exit 1
cp "$B1" /tmp/jit_opt1             || { echo "copy opt1 failed ($B1)" >&2; exit 1; }
echo "Building opt2 (ts_in_xmm: 8 GPRs + XMM timestamps)..." >&2
B2="$(build_bin "jit ts_in_xmm")"  || exit 1
