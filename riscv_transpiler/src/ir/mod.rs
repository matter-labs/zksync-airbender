        ...
        let instruction = match op {
            ...
            OP_IMM_SUBMASK => {
                ...
                let instr = match funct3 {
                    ...
                    _ => panic_or_illegal!(illegal_instr, "Unknown opcode 0x{:08x}", opcode,),
                };
            ...
            OPCODE_SYSTEM => {
                ...
                let instr = if funct3 == ZIMOP_FUNCT3 {
                    ...
                    if funct3 != 0b001 {
                        // not CSRRW
                        panic_or_illegal!(illegal_instr, "Unknown opcode 0x{:08x}", opcode,)
                    } else {
                        instr
                    }
                } else {
                    panic_or_illegal!(illegal_instr, "Unknown system funct3 enc 0x{:08x}", funct3,)
                };
