...
//!
//! Implementation is staged; see `plans/radiant-singing-duckling.md`:
//!   * Stage A  — batched-only emission, unknown dynamic-JALR targets trap.
//!   * Stage B  — second eager copy + dual jump table for unknown JALR targets.
...
pub(crate) fn compute_known_entries(
    ...
    for (i, instr) in program.iter().enumerate() {
        if instr.rd == 1
            && matches!(instr.name, InstructionName::Jal | InstructionName::Jalr)
        {
            let pc = (i as u32) * 4;
...
impl Deferred {
    ...
    pub(crate) fn flush(&mut self, ops: &mut x64::Assembler) {
        ...
        for r in 0..32usize {
            ...
            if k == 0 {
                dynasm!(ops
                    ; mov [rsp + 8 * (r as i32) + rts], r8
                );
            } else {
                dynasm!(ops
                    ; lea rax, [r8 + k]
                    ; mov [rsp + 8 * (r as i32) + rts], rax
                );
            }
        }
        if self.pending != 0 {
    ...
    for j in 0..l {
        ...
        if is_load {
            ...
            dynasm!(ops
                ...
                ; mov DWORD [rsi + Rq(SCRATCH_REGISTER)], Rd(value)
                ; mov rbp, [rsi + tso + 2 * Rq(SCRATCH_REGISTER)]
                ; lea rdx, [r8 + mem_off]
                ; mov [rsi + tso + 2 * Rq(SCRATCH_REGISTER)], rdx
                ; mov [rdi + r9 * 4 + voff], eax
                ; mov [rdi + r9 * 8 + toff], rbp
            );
            accum.bump(2);
            accum.count(CounterType::MemWord, 1);
...
// ---------------------------------------------------------------------------
// Constructor (STAGE A.0 stub: delegates to the eager path so the harness,
// module wiring, and macro/helper reuse can be validated as bit-exact before the
// real lazy emitter lands).
// ---------------------------------------------------------------------------

impl<I: ContextImpl> JittedCode<I> {
    /// STAGE A: single batched copy. Dynamic-JALR targets that are not known
    /// region entries trap (their `jump_offsets` slot points at `->exit_with_error`).
    pub fn preprocess_bytecode_lazy(
        ...
        let instruction_labels = (0..program.len())
            ...
            .collect::<Vec<_>>();
        let mut jump_offsets = vec![0usize; program.len()];
        let mut initialized: HashSet<usize> = HashSet::new();

        let ts_bound = cycles_bound
            .map(|cb| (cb as u64) * TIMESTAMP_STEP + INITIAL_TIMESTAMP);
        ...
        while i < program.len() {
            ...
            if fusion_enabled && matches!(instr.name, Op::Lw | Op::Sw) {
            ...
            match instr.name {
                ...
                Op::Lb | Op::Lbu | Op::Lh | Op::Lhu | Op::Lw => {
                    ...
                    if matches!(instr.name, Op::Lw) {
                        ...
                        touch_register_and_increment_timestamp!(ops, rs1);
                        match instr.name {
                            Op::Lb => dynasm!(ops ; movsx Rd(out), BYTE [rsi + Rq(SCRATCH_REGISTER)]),
                            Op::Lbu => dynasm!(ops ; movzx Rd(out), BYTE [rsi + Rq(SCRATCH_REGISTER)]),
                            Op::Lh => dynasm!(ops ; movsx Rd(out), WORD [rsi + Rq(SCRATCH_REGISTER)]),
                            Op::Lhu => dynasm!(ops ; movzx Rd(out), WORD [rsi + Rq(SCRATCH_REGISTER)]),
                            _ => unreachable!(),
                        }
                        dynasm!(ops
                            ; mov Rd(SCRATCH_REGISTER), DWORD [rsi + 4 * rdx]
                            ; mov rbp, [rsi + (MemoryHolder::TIMESTAMPS_OFFSET as i32) + 8 * rdx]
                            ; mov [rsi + (MemoryHolder::TIMESTAMPS_OFFSET as i32) + 8 * rdx], r8
                            ; mov [rdi + r9 * 4], Rd(SCRATCH_REGISTER)
                            ; mov [rdi + r9 * 8 + (TraceChunk::TIMESTAMPS_OFFSET as i32)], rbp
                        );
                        bump_timestamp!(ops, 1);
                ...
                Op::Sb | Op::Sh | Op::Sw => {
                    ...
                    match instr.name {
                        Op::Sw => dynasm!(ops
                            ; mov eax, DWORD [rsi + Rq(SCRATCH_REGISTER)]
                            ; mov DWORD [rsi + Rq(SCRATCH_REGISTER)], Rd(value)
                            ; mov rbp, [rsi + (MemoryHolder::TIMESTAMPS_OFFSET as i32) + 2 * Rq(SCRATCH_REGISTER)]
                            ; mov [rsi + (MemoryHolder::TIMESTAMPS_OFFSET as i32) + 2 * Rq(SCRATCH_REGISTER)], r8
                            ; mov [rdi + r9 * 4], eax
                            ; mov [rdi + r9 * 8 + (TraceChunk::TIMESTAMPS_OFFSET as i32)], rbp
                        ),
                        Op::Sb => dynasm!(ops
                            ; mov ebp, DWORD [rsi + 4 * rax]
                            ; mov BYTE [rsi + Rq(SCRATCH_REGISTER)], Rb(value)
                            ; push rdx
                            ; mov rdx, [rsi + (MemoryHolder::TIMESTAMPS_OFFSET as i32) + 8 * rax]
                        ...
                        Op::Sh => dynasm!(ops
                            ; mov ebp, DWORD [rsi + 4 * rax]
                            ; mov WORD [rsi + Rq(SCRATCH_REGISTER)], Rw(value)
                            ; push rdx
                            ; mov rdx, [rsi + (MemoryHolder::TIMESTAMPS_OFFSET as i32) + 8 * rax]
                            ; mov [rsi + (MemoryHolder::TIMESTAMPS_OFFSET as i32) + 8 * rax], r8
                            ; mov [rdi + r9 * 4], ebp
                    ...
                    record_circuit_type(
                        &mut ops,
                        if matches!(instr.name, Op::Sw) { CounterType::MemWord } else { CounterType::MemSubword },
                        1,
                    );
                    let pc_for_trace = pc + 4;
                    increment_trace!(ops, pc_for_trace);
                ...
                Op::ZicsrDelegation => {
                    ...
                    let function: *const () = match instr.imm {
                        BLAKE2S_DELEGATION_CSR_REGISTER => {
                            ...
                            record_circuit_type(&mut ops, CounterType::BlakeDelegation, num_calls as u16);
                        ...
                        KECCAK_SPECIAL5_CSR_REGISTER => {
                            ...
                            i += num_calls;
                            cycles_taken = num_calls;
                            record_circuit_type(&mut ops, CounterType::KeccakDelegation, num_calls as u16);
        ...
        // Non-region-entry PCs are not valid batched landings: a dynamic JALR to
        // such a PC traps (STAGE A). Stage B will instead point these at an eager
        // fallback copy.
        for (idx, offset) in jump_offsets.iter_mut().enumerate() {
