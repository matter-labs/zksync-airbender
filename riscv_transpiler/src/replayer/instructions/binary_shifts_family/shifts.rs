...
#[inline(always)]
pub(crate) fn sll<C: Counters, R: RAM>(
    state: &mut State<C>,
    _ram: &mut R,
    instr: Instruction,
    ...
    let rd = rs1_value.wrapping_shl(rs2_value_to_use);
...
#[inline(always)]
pub(crate) fn srl<C: Counters, R: RAM>(
    state: &mut State<C>,
    _ram: &mut R,
    instr: Instruction,
    tracer: &mut impl WitnessTracer,
    ...
    let (rs2_value, rs2_ts) = read_register_with_ts::<C, 1>(state, instr.rs2);
    let rs2_value_to_use = rs2_value.wrapping_add(instr.imm);
    let rd = rs1_value.wrapping_shr(rs2_value_to_use);
    let (rd_old_value, rd_ts) = write_register_with_ts_for_pure_opcode::<C, 2>(state, instr.rd, rd);
...
#[inline(always)]
pub(crate) fn sra<C: Counters, R: RAM>(
    state: &mut State<C>,
    _ram: &mut R,
    instr: Instruction,
    tracer: &mut impl WitnessTracer,
    ...
    let (rs2_value, rs2_ts) = read_register_with_ts::<C, 1>(state, instr.rs2);
    let rs2_value_to_use = rs2_value.wrapping_add(instr.imm);
    let rd = (rs1_value as i32).wrapping_shr(rs2_value_to_use) as u32;
    let (rd_old_value, rd_ts) = write_register_with_ts_for_pure_opcode::<C, 2>(state, instr.rd, rd);
...
#[inline(always)]
pub(crate) fn rev8<C: Counters, R: RAM>(
    state: &mut State<C>,
    _ram: &mut R,
    instr: Instruction,
    tracer: &mut impl WitnessTracer,
) {
    // Zbb rev8: rd = rs1.swap_bytes(). Formal rs2 is x0 and imm = 0 (set at decode), so the
    // row is operand-identical to an immediate shift with a zero amount.
    // Mirrors `vm::instructions::binary_shifts_family::shifts::rev8`.
    let (rs1_value, rs1_ts) = read_register_with_ts::<C, 0>(state, instr.rs1);
