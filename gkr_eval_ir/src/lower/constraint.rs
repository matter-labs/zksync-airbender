...
pub(super) fn lower_batched_constraint<F: PrimeField>(
    arena: &mut ArenaBuilder,
    out: &mut LayerOut,
...
fn lower_flat_max_quadratic<F: PrimeField>(
    arena: &mut ArenaBuilder,
...
fn lower_batched_expr<F: PrimeField>(
    arena: &mut ArenaBuilder,
    input: &NoFieldMaxQuadraticConstraintsGKRRelation<F>,
) -> ExprId {
    let mut terms = Vec::new();
    ...
    for ((a_addr, b_addr), coeff_powers) in input.quadratic_terms.iter() {
        ...
        for (c, p) in coeff_powers.iter() {
            ...
            let scaled_ab = apply_coeff(arena, c.as_u32_reduced(), ab);
    ...
    for (a_addr, coeff_powers) in input.linear_terms.iter() {
        ...
        for (c, p) in coeff_powers.iter() {
            let rho_p = rho_pow(arena, *p);
            let scaled_a = apply_coeff(arena, c.as_u32_reduced(), a_expr);
    ...
    for (c, p) in input.constants.iter() {
        let rho_p = rho_pow(arena, *p);
        terms.push(apply_coeff(arena, c.as_u32_reduced(), rho_p));
    }
