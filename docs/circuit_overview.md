...
- Appending relations: each `append_*` function records a relation plus its `exec_flag` and assigns witnesses early via `cs.set_values`, often using `witness_early_branch_if_possible` to skip work on inactive branches.
...
- For each state component, the following constraint is enforced on every row except the last two:
