...
// 2^26, first LDE 64 => base RS codeword 2^32, folds [3,5,5,5,4] (=> 16 final
// coeffs), every codeword pinned at 2^32, 5 rounds, 100-bit pessimistic security.
// Verifies `proth120_whir_calldata_agg.hex`. Every checked relation is a real assert
// via sink2() (reverts on mismatch). The alternative folds [1,4,5,5,5] schedule (64
// final coeffs) is in the sibling `whir_agg2.sol` / `WhirVerifierAgg2`.
//
...
contract WhirVerifierAgg {
    ...
    // folds [3,5,5,5,4] => 2^4 = 16 final coeffs). See the schedule switch below.
    ...
    uint256 constant MONO_PTR     = 22752;  // final monomials (rfin 4 => 16, <=16*32)
    ...
    fallback() external {
        ...
        // Aggressive schedule: message 2^26, folds [3,5,5,5,4] => final poly 2^4 = 16
        // monomials (rfin 4).
        let nz := 26
        let gcount := 9
        let rfin := 4
        ...
        for { let r := 0 } lt(r, NUM_ROUNDS) { r := add(r, 1) } {
            ...
            // => base RS codeword 2^32, first fold by 8 then fold by 32 while possible
            // down to 16 final coeffs. Every RS codeword is pinned at 2^32, so the LDE
            // factor grows as the message folds: qib_r = 32 - fold_r, cb_r = log2(LDE_r).
            // 100-bit security under the pessimistic conjecture, PoW <= 30:
            //   folds=[3,5,5,5,4] cb=[6,9,14,19,24] q=[14,10,6,5,4] pow=[30,25,30,21,20]
            switch r
            case 0 { fold := 3 q := 14 pow_bits := 30 qib := 29 vp := 8  cb := 6 }
            case 1 { fold := 5 q := 10 pow_bits := 25 qib := 27 vp := 32 cb := 9 }
            case 2 { fold := 5 q := 6  pow_bits := 30 qib := 27 vp := 32 cb := 14 }
            case 3 { fold := 5 q := 5  pow_bits := 21 qib := 27 vp := 32 cb := 19 }
            default { fold := 4 q := 4 pow_bits := 20 qib := 28 vp := 16 cb := 24 }
