...
// =============================================================================
//
// EVM/Yul WHIR proximity-test verifier, sibling of gkr.sol. Every checked relation is a real
// assert via sink2() (reverts on mismatch). The circuit-independent logic is hand-written; the
// per-round fold/query/PoW schedule + the config-derived constants (RS-codeword generators, cap,
// base geometry) are generated by `verifier_evm::generate_verifiers` from the prover's config.
//
...
//
// ---- Layout: merged memory+witness base oracle + setup base oracle ----------
// The witness oracle batches the merged mem+wit columns into one Merkle tree, the setup oracle
// the merged setup columns into another; round 0 opens both (gamma-batched), later rounds fold
// the single intermediate oracle. Message size = 2^(trace_log2 + pack_log2), RS codeword =
// message << base_lde_bits — both fixed by the generated schedule.
// =============================================================================
...
contract WhirVerifier {
    ...
    // RS-codeword evaluation-domain generator and its inverse — a primitive 2^codeword_log2-th
    // root of unity. Generated (`Proth120::TWO_ADICITY_GENERATORS[codeword_log2]`); the values
    // below are placeholders substituted for the target's codeword size.
    ...
    fallback() external {
        ...
        // Base geometry (all generated): nz = z-coords, gcount = gamma-batched base columns + 1,
        // rfin = final-poly log2, nbcaps = base-oracle caps (witness, setup).
...
// =============================================================================
// Test harness (stripped from the generated production verifier). Feeds the REAL prover-generated
// calldata for the Proth120 proof. The verifier reverts on any failed relation, so a successful
// call == a valid proof.
...
contract WhirRealProofTest is WhirVerifier {
    ...
    // preimage length: 64 + ceil(nz/2)*32 + nbcaps*CAP*32 = 64 + 13*32 + 2*8*32 = 992
