...
use crate::cs::gkr_circuits::delegation::keccak_theta_rho::*;
...
pub struct KeccakThetaRhoDelegationCircuit;

impl<F: PrimeField> circuit_common::DelegationCircuit<F> for KeccakThetaRhoDelegationCircuit {
    const DELEGATION_TYPE_ID: u16 =
        common_constants::delegation_types::keccak_f1600::KECCAK_THETA_RHO_CSR_REGISTER as u16;
    const DOMAIN_SIZE_LOG2: u32 = 22;

    fn circuit_fn<CS: cs::cs::circuit_trait::Circuit<F>>(cs: &mut CS) {
        define_keccak_theta_rho_delegation_circuit(cs);
    }

    fn table_addition_fn<CS: cs::cs::circuit_trait::Circuit<F>>(cs: &mut CS) {
        keccak_theta_rho_delegation_circuit_table_addition_fn(cs)
    }
...
pub fn witness_eval_fn(
    proxy: &'_ mut ColumnMajorWitnessProxy<'_, KeccakThetaRhoDelegationOracle<'_>, BabyBearField>,
) {
    let fn_ptr = sealed::evaluate_witness_fn::<
        ScalarWitnessTypeSet<BabyBearField, true>,
        ColumnMajorWitnessProxy<'_, KeccakThetaRhoDelegationOracle<'_>, BabyBearField>,
    >;
    (fn_ptr)(proxy);
}
...
mod test {
    ...
    fn generate() {
        skip_if_ci!();
        circuit_common::generate_default_delegation_artifacts::<KeccakThetaRhoDelegationCircuit>(
            true,
        );
    }
}
