...
use crate::cs::gkr_circuits::mem_word_only::*;
...
pub struct LoadStoreWordOnlyCircuit;

impl<F: PrimeField> circuit_common::RiscVCycleCircuit<F, true> for LoadStoreWordOnlyCircuit {
    ...
    fn circuit_fn<CS: cs::cs::circuit_trait::Circuit<F>>(cs: &mut CS, _bytecode: &[u32]) {
        mem_word_only_circuit_with_preprocessed_bytecode_for_gkr(cs);
    }
...
mod test {
    ...
    fn generate() {
        skip_if_ci!();
        circuit_common::generate_default_risc_v_with_mem_cycles_artifacts::<LoadStoreWordOnlyCircuit>(
            true,
        );
    }
}
