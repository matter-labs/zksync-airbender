...
use crate::cs::gkr_circuits::mul_div::*;
...
pub struct UnsignedMulDivCircuit;
...
impl<F: PrimeField> circuit_common::RiscVCycleCircuit<F, false> for UnsignedMulDivCircuit {
    ...
    fn circuit_fn<CS: cs::cs::circuit_trait::Circuit<F>>(cs: &mut CS, _bytecode: &[u32]) {
        mul_div_circuit_with_preprocessed_bytecode_for_gkr::<_, _, false>(cs);
    }

    fn table_addition_fn<CS: cs::cs::circuit_trait::Circuit<F>>(cs: &mut CS, _bytecode: &[u32]) {
        mul_div_table_addition_fn::<_, _, false>(cs)
    }

    fn table_driver_fn(table_driver: &mut TableDriver<F>, _bytecode: &[u32]) {
        mul_div_table_driver_fn::<_, false>(table_driver);
    }
}
...
mod test {
    ...
    fn generate() {
        skip_if_ci!();
        circuit_common::generate_default_risc_v_non_mem_cycles_artifacts::<UnsignedMulDivCircuit>(
            true,
        );
    }
}
