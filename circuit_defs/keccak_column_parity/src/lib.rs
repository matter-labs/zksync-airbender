...
use crate::cs::gkr_circuits::delegation::keccak_column_parity::*;
...
pub struct KeccakColumnParityDelegationCircuit;
...
impl<F: PrimeField> circuit_common::DelegationCircuit<F> for KeccakColumnParityDelegationCircuit {
    ...
    fn circuit_fn<CS: cs::cs::circuit_trait::Circuit<F>>(cs: &mut CS) {
        define_keccak_column_parity_delegation_circuit(cs);
    }

    fn table_addition_fn<CS: cs::cs::circuit_trait::Circuit<F>>(cs: &mut CS) {
        keccak_column_parity_delegation_circuit_table_addition_fn(cs)
    }
...
pub fn witness_eval_fn(
    proxy: &'_ mut ColumnMajorWitnessProxy<
        '_,
        KeccakColumnParityDelegationOracle<'_>,
        BabyBearField,
    >,
) {
    let fn_ptr = sealed::evaluate_witness_fn::<
        ScalarWitnessTypeSet<BabyBearField, true>,
        ColumnMajorWitnessProxy<'_, KeccakColumnParityDelegationOracle<'_>, BabyBearField>,
    >;
    (fn_ptr)(proxy);
}
...
mod test {
    ...
    fn generate() {
        skip_if_ci!();
        circuit_common::generate_default_delegation_artifacts::<KeccakColumnParityDelegationCircuit>(
            true,
        );
    }
}
