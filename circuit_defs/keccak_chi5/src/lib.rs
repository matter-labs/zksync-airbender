...
use crate::cs::gkr_circuits::delegation::keccak_chi5::*;
...
pub struct KeccakChi5DelegationCircuit;

impl<F: PrimeField> circuit_common::DelegationCircuit<F> for KeccakChi5DelegationCircuit {
    const DELEGATION_TYPE_ID: u16 =
        common_constants::delegation_types::keccak_f1600::KECCAK_CHI5_CSR_REGISTER as u16;
    const DOMAIN_SIZE_LOG2: u32 = 22;

    fn circuit_fn<CS: cs::cs::circuit_trait::Circuit<F>>(cs: &mut CS) {
        define_keccak_chi5_delegation_circuit(cs);
    }
...
pub fn witness_eval_fn(
    proxy: &'_ mut ColumnMajorWitnessProxy<'_, KeccakChi5DelegationOracle<'_>, BabyBearField>,
) {
    let fn_ptr = sealed::evaluate_witness_fn::<
        ScalarWitnessTypeSet<BabyBearField, true>,
        ColumnMajorWitnessProxy<'_, KeccakChi5DelegationOracle<'_>, BabyBearField>,
    >;
    (fn_ptr)(proxy);
}
...
mod test {
    ...
    fn generate() {
        skip_if_ci!();
        circuit_common::generate_default_delegation_artifacts::<KeccakChi5DelegationCircuit>(true);
    }
}
