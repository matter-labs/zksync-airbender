    ...
    fn draw_random_field_elements_with_pow(
        ...
        let mut state = TranscriptState::<REDUCED_ROUNDS>::new(new_seed);
        {
            ...
            for slot in buffer.iter_mut() {
                // a zero element gives us a correctly-sized `Coeffs` to overwrite,
                // avoiding `[F; E::DEGREE]` (which would need `generic_const_exprs`).
                let mut coeffs: [BabyBearField; 4] =
                    <BabyBearExt4 as FieldExtension<BabyBearField>>::into_coeffs(
                        BabyBearExt4::ZERO,
                    );
                for c in coeffs.as_mut().iter_mut() {
                    *c = BabyBearField::from_raw_repr_with_reduction(
                        it.next().expect("transcript word"),
                    );
                }
