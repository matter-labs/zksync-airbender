...
.agents/bin/with_gpu_lock.sh ncu \
  --nvtx \
  --nvtx-include 'test.gpu.prove.profiled_call@gpu_prover.tests' \
  --set basic \
  --kernel-name-base demangled \
  --kernel-name 'regex:<kernel_regex>' \
  --launch-skip <matching_launches_to_skip> \
  --launch-count <matching_launches_to_collect> \
  -o target/profiling/ncu/gpu_prover_kernel \
  "$TEST_BINARY" \
...
```bash
GPU_PROVER_ENABLE_LINEINFO=1 cargo test -p gpu_prover run_basic_unrolled_proof_job_profile_test --release --no-run --message-format=json \
...
.agents/bin/with_gpu_lock.sh ncu \
  ...
  --nvtx-include 'test.gpu.prove.profiled_call@gpu_prover.tests' \
  --import-source yes \
  --source-folders gpu_prover/native \
  ...
  -o target/profiling/ncu/gpu_prover_full \
  "$TEST_BINARY" \
  --exact prover::tests::run_basic_unrolled_proof_job_profile_test \
  --ignored \
  --nocapture
...
.agents/bin/with_gpu_lock.sh ncu \
  --nvtx \
  --nvtx-include 'profile.tmp.<kernel_group>@gpu_prover.tests' \
  --replay-mode range \
  --cache-control none \
  --import-source yes \
  --source-folders gpu_prover/native \
  ...
  --section WarpStateStats \
  --section WorkloadDistribution \
  -o target/profiling/ncu/gpu_prover_range \
  "$TEST_BINARY" \
  --exact prover::tests::run_basic_unrolled_proof_job_profile_test \
  --ignored \
  --nocapture
