...
mod scaffolding_unary {
    ...
    where
        Dbl: UnaryOp<T>,
    {
        let device_fn = crate::ops::simple::dbl;
    ...
    where
        Dbl: UnaryOp<T>,
    {
        let device_fn = crate::ops::simple::dbl_in_place;
        let host_fn = |x: &T| *x.clone().double();
    ...
    where
        Inv: UnaryOp<T>,
    {
        let device_fn = crate::ops::simple::inv;
    ...
    where
        Inv: UnaryOp<T>,
    {
        let device_fn = crate::ops::simple::inv_in_place;
        let host_fn = |x: &T| x.inverse().unwrap_or(T::ZERO);
        super::unary_op_in_place_test::<T, LOG_N>(device_fn, host_fn, additional_values);
    }
    ...
    where
        Neg: UnaryOp<T>,
    {
        let device_fn = crate::ops::simple::neg;
    ...
    where
        Neg: UnaryOp<T>,
    {
        let device_fn = crate::ops::simple::neg_in_place;
        let host_fn = |x: &T| *x.clone().negate();
        super::unary_op_in_place_test::<T, LOG_N>(device_fn, host_fn, additional_values);
    }
    ...
    where
        Sqr: UnaryOp<T>,
    {
        let device_fn = crate::ops::simple::sqr;
    ...
    where
        Sqr: UnaryOp<T>,
    {
        let device_fn = crate::ops::simple::sqr_in_place;
        let host_fn = |x: &T| *x.clone().square();
        super::unary_op_in_place_test::<T, LOG_N>(device_fn, host_fn, additional_values);
    }
