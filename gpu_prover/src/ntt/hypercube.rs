...
strided_tiles_stages!(ab_hypercube_monomials_to_evals_last_9_stages_kernel);
strided_tiles_stages!(ab_hypercube_monomials_to_evals_last_10_stages_kernel);
...
strided_tiles_stages!(ab_hypercube_monomials_to_evals_noninitial_8_stages_kernel);
...
// 2-pass evals to monomials
contiguous_chunks_stages!(ab_hypercube_evals_to_monomials_last_14_stages_kernel);

// 2-pass monomials to evals
contiguous_chunks_stages!(ab_hypercube_monomials_to_evals_first_14_stages_kernel);

// 3-pass evals to monomials
contiguous_chunks_stages!(ab_hypercube_evals_to_monomials_final_5_stages_kernel);
contiguous_chunks_stages!(ab_hypercube_evals_to_monomials_final_6_stages_kernel);
contiguous_chunks_stages!(ab_hypercube_evals_to_monomials_final_7_stages_kernel);
contiguous_chunks_stages!(ab_hypercube_evals_to_monomials_final_8_stages_kernel);

// 3-pass monomials to evals
contiguous_chunks_stages!(ab_hypercube_monomials_to_evals_initial_5_stages_kernel);
contiguous_chunks_stages!(ab_hypercube_monomials_to_evals_initial_6_stages_kernel);
contiguous_chunks_stages!(ab_hypercube_monomials_to_evals_initial_7_stages_kernel);
contiguous_chunks_stages!(ab_hypercube_monomials_to_evals_initial_8_stages_kernel);
...
pub(crate) fn hypercube_evals_to_monomials_3_pass(
    ...
    for col in 0..num_ntts {
        ...
        match log_n {
            21 => ContiguousChunksStagesFunction(
                ab_hypercube_evals_to_monomials_final_5_stages_kernel,
            )
            .launch(&config, &args)?,
            22 => ContiguousChunksStagesFunction(
                ab_hypercube_evals_to_monomials_final_6_stages_kernel,
            )
            .launch(&config, &args)?,
            23 => ContiguousChunksStagesFunction(
                ab_hypercube_evals_to_monomials_final_7_stages_kernel,
            )
            .launch(&config, &args)?,
            24 => ContiguousChunksStagesFunction(
                ab_hypercube_evals_to_monomials_final_8_stages_kernel,
            )
            .launch(&config, &args)?,
            _ => unimplemented!(),
        }
...
pub(crate) fn hypercube_evals_to_monomials_2_pass(
    ...
    for col in 0..num_ntts {
        ...
        let function =
            ContiguousChunksStagesFunction(ab_hypercube_evals_to_monomials_last_14_stages_kernel);
        let func_ptr = function.as_ptr();
        unsafe {
            cudaFuncSetAttribute(
                func_ptr,
...
pub(crate) fn hypercube_monomials_to_evals_3_pass(
    inputs_matrix: &(impl DeviceMatrixChunkImpl<BF> + ?Sized),
    outputs_matrix: &mut (impl DeviceMatrixChunkMutImpl<BF> + ?Sized),
    log_n: usize,
    transposed_monomials: bool,
    ...
    for col in 0..num_ntts {
        ...
        match log_n {
            21 => ContiguousChunksStagesFunction(
                ab_hypercube_monomials_to_evals_initial_5_stages_kernel,
            )
            .launch(&config, &args)?,
            22 => ContiguousChunksStagesFunction(
                ab_hypercube_monomials_to_evals_initial_6_stages_kernel,
            )
            .launch(&config, &args)?,
            23 => ContiguousChunksStagesFunction(
                ab_hypercube_monomials_to_evals_initial_7_stages_kernel,
            )
            .launch(&config, &args)?,
            24 => ContiguousChunksStagesFunction(
                ab_hypercube_monomials_to_evals_initial_8_stages_kernel,
            )
            .launch(&config, &args)?,
            _ => unimplemented!(),
        }
        ...
        for _ in 0..2 {
            ...
            StridedTilesStagesFunction(ab_hypercube_monomials_to_evals_noninitial_8_stages_kernel)
                .launch(&config, &args)?;
            start_stage += 8;
        }
    }
...
pub(crate) fn hypercube_monomials_to_evals_2_pass(
    inputs_matrix: &(impl DeviceMatrixChunkImpl<BF> + ?Sized),
    outputs_matrix: &mut (impl DeviceMatrixChunkMutImpl<BF> + ?Sized),
    log_n: usize,
    transposed_monomials: bool,
    ...
    for col in 0..num_ntts {
        ...
        let function =
            ContiguousChunksStagesFunction(ab_hypercube_monomials_to_evals_first_14_stages_kernel);
        let func_ptr = function.as_ptr();
        unsafe {
            cudaFuncSetAttribute(
                func_ptr,
        ...
        let function = match log_n {
            23 => StridedTilesStagesFunction(ab_hypercube_monomials_to_evals_last_9_stages_kernel),
            24 => StridedTilesStagesFunction(ab_hypercube_monomials_to_evals_last_10_stages_kernel),
            _ => unimplemented!(),
        };
        let func_ptr = function.as_ptr();
        unsafe {
...
pub fn hypercube_monomials_to_evals(
    ...
    if l2_bytes > column_bytes {
        hypercube_monomials_to_evals_3_pass(
            inputs_matrix,
            outputs_matrix,
            log_n,
            transposed_monomials,
        ...
        hypercube_monomials_to_evals_2_pass(
            inputs_matrix,
            outputs_matrix,
            log_n,
            transposed_monomials,
