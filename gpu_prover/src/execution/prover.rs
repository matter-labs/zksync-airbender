...
use crate::execution::gpu_manager::GpuManager;
use crate::execution::messages::{
    FinalRegisterValue, GpuWorkBatch, GpuWorkRequest, GpuWorkResult, InitsAndTeardownsData,
    MemoryCommitmentRequest, MemoryCommitmentResult, ProofRequest, ProofResult, SimulationResult,
    TracingData, WorkerResult,
};
use crate::execution::precomputations::{
...
/// - `Unified`: the reduced-machine unified circuit. Public surface is
///   wired up; the GPU dispatch points panic with `unimplemented!()` until
///   unified is implemented (per the porting plan's "ship Unified as
///   first-class API but `unimplemented!()` at GPU dispatch" rule).
...
impl ExecutionProver {
    ...
    pub fn with_configuration(configuration: ExecutionProverConfiguration) -> Self {
        ...
        // Common precomputations cover the binary-independent circuits: every
        // delegation circuit and inits-and-teardowns. They depend on the WHIR
        // schedule we choose at `add_binary` time but those four logs are
        // also fixed per circuit-family in the new API, so we can compute
    ...
    pub fn add_binary(
        ...
        setups::pad_bytecode_for_proving(&mut padded_binary_image);
        let mut padded_text_section = text_section.clone();
        setups::pad_bytecode_for_proving(&mut padded_text_section);
    ...
    fn get_result(
        ...
        // For Proof requests, the per-coset caps from a prior
        // commit_memory phase, keyed by (CircuitType, sequence_id). Empty
        // for the commit_memory-only path.
        ...
        let get_gpu_work_request = |inits_and_teardowns: Option<InitsAndTeardownsData>,
                ...
                WorkerResult::GpuWorkResult(result) => {
                    ...
                    match result {
                        GpuWorkResult::MemoryCommitment(commitment) => {
                            ...
                            let caps: &mut BTreeMap<usize, Vec<MerkleTreeCapVarLength>> =
                                match circuit_type {
    ...
    fn free_inits_and_teardowns(&self, inits_and_teardowns: InitsAndTeardownsTraceHost) {
    ...
    pub fn prove(
        ...
        unimplemented!(
            "ExecutionProver::prove requires prior memory commitments; use commit_memory_and_prove()"
        )
    }
    ...
    pub fn commit_memory_and_prove(
        ...
        // Build a (CircuitType, sequence_id) → per-coset caps map from the
        // commit phase results, so the prove phase's ProofRequests can carry
        // the caps the new GKR `prove()` requires.
...
/// Map an unrolled-circuit `family_idx` (8-bit family identifier from the
/// commit-phase cap maps) back to the concrete `UnrolledCircuitType` for
/// the active machine type. The orchestrator's commit phase stores caps
/// keyed by family idx; the prove phase needs caps keyed by full
/// CircuitType, hence this small reverse lookup.
