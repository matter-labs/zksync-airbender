...
#[derive(Default)]
struct FrontChunksCache<A: GoodAllocator> {
    main_chunks: HashMap<usize, TracingDataHost<A>>,
    delegation_chunks: HashMap<DelegationCircuitType, HashMap<usize, TracingDataHost<A>>>,
}

impl<A: GoodAllocator> FrontChunksCache<A> {
    fn get_chunks_count(&self) -> usize {
        self.main_chunks.len()
            + self
                .delegation_chunks
                .values()
                .map(|v| v.len())
...
impl<K: Clone + Debug + Eq + Hash> ExecutionProver<'_, K> {
    ...
    fn get_results(
        &self,
        proving: bool,
        front_chunks_cache: &mut Option<FrontChunksCache<A>>,
        batch_id: u64,
        binary_key: &K,
        ...
        let main_skip_set = front_chunks_cache
            .as_ref()
            .map(|c| {
                c.main_chunks
                    .iter()
            ...
            .unwrap_or_default();
        let delegation_skip_sets = front_chunks_cache
            .as_ref()
            .map(|c| {
                c.delegation_chunks
                    .iter()
                    .map(|(t, s)| (*t, s.iter().map(|(i, _)| *i).collect::<HashSet<usize>>()))
                    .collect::<HashMap<DelegationCircuitType, HashSet<usize>>>()
            })
            .unwrap_or_default();
        let mut main_work_requests_count = 0;
        if proving {
            if let Some(cache) = front_chunks_cache.take() {
                let FrontChunksCache {
                    main_chunks,
                    delegation_chunks,
                } = cache;
                for (circuit_sequence, tracing_data) in main_chunks.into_iter() {
                    ...
                    trace!("BATCH[{batch_id}] PROVER sending cached main circuit proof request for chunk {circuit_sequence} to GPU manager");
                    gpu_work_requests_sender.send(request).unwrap();
                    main_work_requests_count += 1;
                }
                for (delegation_type, tracing_data_map) in delegation_chunks.into_iter() {
                    let circuit_type = CircuitType::Delegation(delegation_type);
                    let precomputations =
                        self.delegation_circuits_precomputations[&delegation_type].clone();
                    for (circuit_sequence, tracing_data) in tracing_data_map.into_iter() {
                        let request = ProofRequest {
                            batch_id,
                            circuit_type,
                            circuit_sequence,
                        ...
                        let request = GpuWorkRequest::Proof(request);
                        trace!("BATCH[{batch_id}] PROVER sending cached delegation proof request for delegation {:?} to GPU manager", delegation_type);
                        gpu_work_requests_sender.send(request).unwrap();
                    }
                }
            }
        ...
        let delegation_mode = CpuWorkerMode::TraceDelegations {
            skip_sets: delegation_skip_sets,
                ...
                WorkerResult::MemoryCommitment(commitment) => {
                    ...
                    match tracing_data {
                        TracingDataHost::Main {
                            ...
                            if front_chunks_cache
                                .as_ref()
                                .map_or(false, |cache| cache.get_chunks_count() < self.device_count)
                            {
                                trace!(
                                    "BATCH[{batch_id}] PROVER caching trace for main circuit {:?} chunk {}",
                                    circuit_type,
                                    circuit_sequence
                                );
                                ...
                                assert!(front_chunks_cache
                                    .as_mut()
                                    .unwrap()
                                    .main_chunks
                                    .insert(circuit_sequence, data)
                                    .is_none());
                            } else {
                        ...
                        TracingDataHost::Delegation(witness) => {
                            ...
                            {
                                trace!(
                                    "BATCH[{batch_id}] PROVER caching trace for delegation circuit {:?}",
                                    circuit_type,
                                );
    ...
    pub fn commit_memory_and_prove(
        ...
        assert_eq!(
            front_chunks_cache.as_ref().unwrap().get_chunks_count(),
            min(
                self.device_count,
                main_memory_commitments.len() + delegation_memory_commitments.len()
            )
        );
        ...
        assert!(front_chunks_cache.is_none());
        info!(
