...
use crate::execution::messages::FinalRegisterValue;

/// Sparse init-and-teardown record produced by the memory-holder traversal.
///
/// Replaces the legacy `prover::definitions::LazyInitAndTeardown`, which is no
/// longer exposed by the active prover (its sole consumer in
/// `prover/src/gkr/witness_gen/family_circuits/unified.rs` is commented out).
    ...
    *non_determinism_guard = Some(non_determinism_source);
    ...
    if !should_abort {
        ...
        let (circuit_type, sequence_id_offset) = if T::IS_SPLIT {
            ...
            // sequence_ids span the full circuit count (mirroring legacy
            // `cpu_worker.rs:98-117`). The orchestrator and replayer
            // assume `sequence_id` runs `0..total_circuits` even for
            // circuits that have no I&T data; without these prefill
            // markers, sequence_ids would skip and the replayer's tracing
            // results would not pair up.
            // Per F5: under the new prover's `2^N`-row convention each
            // unified circuit covers `domain_size` cycles (one cycle per
            // usable row).
...
fn get_inits_and_teardowns_chunks(
    ...
    // Drain pages from the BTreeMap partition by partition. Mirroring
    // legacy `get_inits_and_teardowns_chunks`, the *first* partition
    // absorbs the remainder and later partitions are full-sized; keeps
    // sequence_id assignment stable.
