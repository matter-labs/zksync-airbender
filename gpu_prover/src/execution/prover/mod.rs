...
use crate::execution::workers::cpu::{run_replayer, run_simulator};
use crate::execution::workers::gpu_manager::GpuManager;
...
impl ExecutionProver {
    ...
    pub fn with_configuration(configuration: ExecutionProverConfiguration) -> Self {
        ...
        // delegation circuit and inits-and-teardowns. The four WHIR logs are
        // fixed per circuit-family, so we can compute them once up front.
        // The inits-and-teardowns entry uses logs (24 / 1 / 4); delegation
        // entries pick their own per-family.
    ...
    /// Derives the per-batch `(pow_challenge, external_challenges, proof_caps)`
    /// from a memory-commitment result. Pure derivation — no side effects.
    fn derive_proof_artifacts(
        ...
        // Build a (CircuitType, sequence_id) → per-coset caps map so the prove
        // phase's ProofRequests can carry the caps the inner pipeline requires.
    ...
    /// Produces a proof for a previously committed memory state. Caller is
    /// responsible for supplying a non-determinism source that yields the same
    /// reads the commit phase observed; for chained commit→prove on a single
    /// source, [`commit_memory_and_prove`] handles the replay internally.
    pub fn prove(
        &self,
        batch_id: u64,
...
/// Map an `family_idx` (8-bit family identifier) back to the concrete
/// `UnrolledCircuitType` for the active machine type. Commit-phase caps
/// are keyed by family idx; the prove phase needs them keyed by
/// `CircuitType`.
