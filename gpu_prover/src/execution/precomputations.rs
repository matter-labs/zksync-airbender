...
use crate::primitives::circuit_type::UnrolledCircuitType::InitsAndTeardowns;
    ...
    F: Fn(CircuitType) -> (u32, u32, u32),
{
        ...
        .copied()
    {
        let setup = match delegation_type {
            DelegationCircuitType::BigIntWithControl => {
                setups::circuits::get_bigint_with_control_circuit_setup(true, worker)
            }
    ...
    let it_setup = setups::unrolled_circuits::inits_and_teardowns_circuit_setup::<
...
fn build_unrolled_setup(
    ...
    match circuit_type {
        UnrolledCircuitType::Memory(UnrolledMemoryCircuitType::LoadStoreWordOnly) => {
            let s = setups::unrolled_circuits::load_store_word_only_circuit_setup::<Global>(
                &table_data,
                binary_image,
                true,
                worker,
        ...
        UnrolledCircuitType::Memory(UnrolledMemoryCircuitType::LoadStoreSubwordOnly) => {
            let s = setups::unrolled_circuits::load_store_subword_only_circuit_setup::<Global>(
                &table_data,
                binary_image,
                true,
                worker,
        ...
        UnrolledCircuitType::NonMemory(UnrolledNonMemoryCircuitType::AddSubLuiAuipcMop) => {
            let s = setups::unrolled_circuits::add_sub_lui_auipc_mop_circuit_setup::<Global>(
                &table_data,
                true,
                worker,
            );
        ...
        UnrolledCircuitType::NonMemory(UnrolledNonMemoryCircuitType::JumpBranchSlt) => {
            let s = setups::unrolled_circuits::jump_branch_slt_circuit_setup::<Global>(
                &table_data,
                true,
                worker,
            );
        ...
        UnrolledCircuitType::NonMemory(UnrolledNonMemoryCircuitType::ShiftBinaryCsr) => {
            let s = setups::unrolled_circuits::shift_binary_circuit_setup::<Global>(
                &table_data,
                true,
                worker,
            );
        ...
        UnrolledCircuitType::NonMemory(UnrolledNonMemoryCircuitType::MulDivUnsigned) => {
            let s = setups::unrolled_circuits::mul_div_unsigned_circuit_setup::<Global>(
                &table_data,
                true,
                worker,
            );
