...
EXTERN __device__ __constant__ powers_data_3_layer powers_data_w;
EXTERN __device__ __constant__ powers_data_2_layer powers_data_w_bitrev_for_ntt;
EXTERN __device__ __constant__ powers_data_2_layer powers_data_w_inv_bitrev_for_ntt;
...
DEVICE_FORCEINLINE ext2_field get_power(const powers_data_3_layer &data, const unsigned index, const bool inverse) {
  ...
  ext2_field val = memory::load_ca(data.coarsest.values + coarsest_idx);

  const unsigned coarser_idx = (idx >> data.fine.log_count) & data.coarser.mask;
  if (coarser_idx != 0)
    val = ext2_field::mul(val, memory::load_ca(data.coarser.values + coarser_idx));

  const unsigned fine_idx = idx & data.fine.mask;
  if (fine_idx != 0)
    val = ext2_field::mul(val, memory::load_ca(data.fine.values + fine_idx));

  return val;
}

DEVICE_FORCEINLINE ext2_field get_power_of_w(const unsigned index, const bool inverse) { return get_power(powers_data_w, index, inverse); }
