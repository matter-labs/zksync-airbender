...
fn leaves(c: &mut Criterion<CudaMeasurement>) {
    ...
    for log_n in MIN_LOG_N..=MAX_LOG_N {
        let hashes_count =
            (COLUMNS_COUNT - BLOCK_SIZE).div_ceil(BLOCK_SIZE) << (log_n + LOG_ROWS_PER_HASH);
...
fn tree(c: &mut Criterion<CudaMeasurement>) {
    ...
    for log_n in MIN_LOG_N..=MAX_LOG_N {
        let hashes_count = (COLUMNS_COUNT << (log_n + LOG_ROWS_PER_HASH)) / BLOCK_SIZE;
        group.throughput(Throughput::Elements(hashes_count as u64));
        group.bench_with_input(
            format!("{}", log_n),
            &log_n,
...
fn pow(c: &mut Criterion<CudaMeasurement>) {
