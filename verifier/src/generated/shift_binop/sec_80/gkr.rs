...
unsafe fn layer_0_final_step_accumulator(
    ...
    let mut current_batch = BabyBearExt4::ONE;
    {
        const SIMPLE_GATES: [(SimpleGateType, [usize; 4]); 5usize] = [
            ...
            (SimpleGateType::Product, [53usize, 54usize, 0usize, 0usize]),
            (SimpleGateType::Product, [55usize, 56usize, 0usize, 0usize]),
        ];
        let mut _sg = 0;
        while _sg < 5usize {
    ...
    }
    {
        const SIMPLE_GATES: [(SimpleGateType, [usize; 4]); 8usize] = [
            (
                SimpleGateType::LookupWithSetup,
                [43usize, 25usize, 47usize, 0usize],
            ),
            (SimpleGateType::Copy, [44usize, 0usize, 0usize, 0usize]),
            (
                SimpleGateType::LookupWithSetup,
                [45usize, 26usize, 48usize, 0usize],
            ),
            (
                SimpleGateType::LookupInitialPair,
                [46usize, 57usize, 0usize, 0usize],
            ),
            (
                SimpleGateType::LookupInitialPair,
            ...
            (
                SimpleGateType::LookupInitialWithCachedDenominators,
                [38usize, 63usize, 27usize, 64usize],
            ),
        ];
        let mut _sg = 0;
        while _sg < 8usize {
...
unsafe fn layer_1_final_step_accumulator(
    ...
    let mut current_batch = BabyBearExt4::ONE;
    {
        const SIMPLE_GATES: [(SimpleGateType, [usize; 4]); 11usize] = [
            ...
            (
                SimpleGateType::LookupUnbalanced,
                [5usize, 6usize, 7usize, 0usize],
            ),
            (
                SimpleGateType::LookupUnbalanced,
                [14usize, 15usize, 16usize, 0usize],
            ),
            (
                SimpleGateType::LookupAggregatePair,
            ...
            (
                SimpleGateType::LookupUnbalanced,
                [17usize, 18usize, 23usize, 0usize],
            ),
        ];
        let mut _sg = 0;
        while _sg < 11usize {
...
fn check_virtual_setup_range_check_16bits<E: ErrorCreator>(
    ...
    unsafe {
        ...
        if result != *state.prev_claims.get_unchecked(68usize) {
            return Err(E::gkr_virtual_setup_eval_mismatch(68usize));
        }
    }
    Ok(())
}
...
fn check_virtual_setup_range_check_timestamp<E: ErrorCreator>(
    ...
    unsafe {
        ...
        if result != *state.prev_claims.get_unchecked(69usize) {
            return Err(E::gkr_virtual_setup_eval_mismatch(69usize));
        }
    }
    Ok(())
}
...
pub(crate) fn verify_gkr<I: NonDeterminismSource<BabyBearField>, E: ErrorCreator>(
    ...
    unsafe {
        ...
        }
        {
            ...
            let fc_len = 24usize;
            let data_words = 12usize * EXT_DEGREE;
            {
                let mut i = 0;
                while i < data_words {
                    eval_buf.data_write(i, read_reduced_field_el::<I>(nd_source).as_u32_raw_repr());
            ...
            {
                let evals: &[[BabyBearExt4; 1]] = eval_buf.data_as(12usize);
        ...
        }
        {
            ...
            let fc_len = 24usize;
            let data_words = 24usize * EXT_DEGREE;
            {
                let mut i = 0;
                while i < data_words {
                    eval_buf.data_write(i, read_reduced_field_el::<I>(nd_source).as_u32_raw_repr());
            ...
            {
                let evals: &[[BabyBearExt4; 1]] = eval_buf.data_as(24usize);
            ...
            {
                let slice: &[BabyBearExt4] = unsafe { eval_buf.data_as(24usize + 40usize) };
                let mut k = 24usize;
                while k < 24usize + 40usize {
            ...
            let next_batching = draw_single_field_el(ts);
            let final_step_evals: &[[BabyBearExt4; 1]] = unsafe { eval_buf.data_as(24usize) };
            state.prev_claims.clear();
            {
        ...
        }
        {
            ...
            let fc_len = 24usize;
            let data_words = 65usize * EXT_DEGREE;
            {
                let mut i = 0;
                while i < data_words {
                    eval_buf.data_write(i, read_reduced_field_el::<I>(nd_source).as_u32_raw_repr());
            ...
            {
                let evals: &[[BabyBearExt4; 1]] = eval_buf.data_as(65usize);
            ...
            {
                let slice: &[BabyBearExt4] = unsafe { eval_buf.data_as(65usize + 21usize) };
                let mut k = 65usize;
                while k < 65usize + 21usize {
                    extra_evals.push(unsafe { *slice.get_unchecked(k) });
                    k += 1;
                }
            }
            ts.commit(&mut eval_buf, data_words + 21usize * EXT_DEGREE);
            let next_batching = draw_single_field_el(ts);
            let final_step_evals: &[[BabyBearExt4; 1]] = unsafe { eval_buf.data_as(65usize) };
            state.prev_claims.clear();
            {
            ...
            }
            {
                ...
                let addr_claim = *state.prev_claims.get_unchecked(6usize);
                ...
                {
                    let low = *state.prev_claims.get_unchecked(2usize);
                    let mut combined = *state.prev_claims.get_unchecked(3usize);
                    field_ops::mul_assign_by_base(
                        &mut combined,
                        &BabyBearField::from_reduced_raw_repr(268434910u32),
                    );
                    field_ops::add_assign(&mut combined, &low);
                    let mut t_val: BabyBearExt4 =
                        external_challenges.permutation_argument_linearization_challenges[4usize];
                    field_ops::mul_assign(&mut t_val, &combined);
                    field_ops::add_assign(&mut expected, &t_val);
                }
                {
                    let low = *state.prev_claims.get_unchecked(4usize);
                    let mut combined = *state.prev_claims.get_unchecked(5usize);
                    field_ops::mul_assign_by_base(
                        &mut combined,
                        &BabyBearField::from_reduced_raw_repr(268434910u32),
                    );
            ...
            }
            {
                ...
                let mut t_addr: BabyBearExt4 =
                    external_challenges.permutation_argument_linearization_challenges[0usize];
                let addr_claim = *state.prev_claims.get_unchecked(13usize);
                field_ops::mul_assign(&mut t_addr, &addr_claim);
                field_ops::add_assign(&mut expected, &t_addr);
                {
                    let mut t_ts: BabyBearExt4 =
                        external_challenges.permutation_argument_linearization_challenges[2usize];
                    let mut ts_low = *state.prev_claims.get_unchecked(7usize);
                    field_ops::add_assign_base(
                        &mut ts_low,
                        &BabyBearField::from_reduced_raw_repr(0u32),
                    );
                ...
                }
                {
                    let mut t_ts: BabyBearExt4 =
                        external_challenges.permutation_argument_linearization_challenges[3usize];
                    let ts_high = *state.prev_claims.get_unchecked(8usize);
                    field_ops::mul_assign(&mut t_ts, &ts_high);
                    field_ops::add_assign(&mut expected, &t_ts);
                }
                {
                    let low = *state.prev_claims.get_unchecked(9usize);
                    let mut combined = *state.prev_claims.get_unchecked(10usize);
                    field_ops::mul_assign_by_base(
                        &mut combined,
                        &BabyBearField::from_reduced_raw_repr(268434910u32),
                    );
                ...
                {
                    let low = *state.prev_claims.get_unchecked(11usize);
                    let mut combined = *state.prev_claims.get_unchecked(12usize);
                    field_ops::mul_assign_by_base(
                        &mut combined,
                        &BabyBearField::from_reduced_raw_repr(268434910u32),
                    );
            ...
            }
            {
                ...
                field_ops::add_assign(&mut expected, &t_addr);
                {
                    let mut t_ts: BabyBearExt4 =
                        external_challenges.permutation_argument_linearization_challenges[2usize];
                    let mut ts_low = *state.prev_claims.get_unchecked(14usize);
                    field_ops::add_assign_base(
                        &mut ts_low,
                        &BabyBearField::from_reduced_raw_repr(0u32),
                    );
                ...
                }
                {
                    let mut t_ts: BabyBearExt4 =
                        external_challenges.permutation_argument_linearization_challenges[3usize];
                    let ts_high = *state.prev_claims.get_unchecked(15usize);
                    field_ops::mul_assign(&mut t_ts, &ts_high);
                    field_ops::add_assign(&mut expected, &t_ts);
                }
                {
            ...
            }
            {
                ...
                let cached = *state.prev_claims.get_unchecked(75usize);
                if expected != cached {
            ...
            }
            {
                ...
                let cached = *state.prev_claims.get_unchecked(76usize);
                if expected != cached {
            ...
            }
            {
                ...
                let cached = *state.prev_claims.get_unchecked(77usize);
                if expected != cached {
