...
unsafe fn layer_0_compute_claim(
    ...
    const DESCS: [(usize, usize, usize); 22usize] = [
...
unsafe fn layer_0_final_step_accumulator(
    ...
    }
    {
        ...
        for j in 0..2 {
            let mut val = BabyBearExt4::from_base(BabyBearField::from_reduced_raw_repr(0u32));
            {
                ...
                field_ops::add_assign(&mut inner, &t);
                let a_val = unsafe { evals.get_unchecked(15usize) }[j];
            ...
            let mut lt = unsafe { evals.get_unchecked(11usize) }[j];
            field_ops::mul_assign_by_base(&mut lt, &BabyBearField::from_reduced_raw_repr(65536u32));
            field_ops::add_assign(&mut val, &lt);
    ...
    }
    {
        ...
        for j in 0..2 {
            ...
            field_ops::mul_assign_by_base(&mut lt, &BabyBearField::from_reduced_raw_repr(1u32));
            field_ops::add_assign(&mut val, &lt);
            let mut lt = unsafe { evals.get_unchecked(12usize) }[j];
            field_ops::mul_assign_by_base(&mut lt, &BabyBearField::from_reduced_raw_repr(65536u32));
            field_ops::add_assign(&mut val, &lt);
            let mut lt = unsafe { evals.get_unchecked(16usize) }[j];
            field_ops::mul_assign_by_base(
    ...
    }
    {
        ...
        for j in 0..2 {
            let mut val = BabyBearExt4::from_base(BabyBearField::from_reduced_raw_repr(0u32));
            {
                ...
                field_ops::add_assign(&mut inner, &t);
                let a_val = unsafe { evals.get_unchecked(13usize) }[j];
                field_ops::mul_assign(&mut inner, &a_val);
                field_ops::add_assign(&mut val, &inner);
            }
            let mut lt = unsafe { evals.get_unchecked(13usize) }[j];
            field_ops::mul_assign_by_base(
                &mut lt,
                &BabyBearField::from_reduced_raw_repr(1744830467u32),
            );
    ...
    }
    {
        ...
        for j in 0..2 {
            let mut val = BabyBearExt4::from_base(BabyBearField::from_reduced_raw_repr(0u32));
            {
                ...
                field_ops::add_assign(&mut inner, &t);
                let a_val = unsafe { evals.get_unchecked(14usize) }[j];
                field_ops::mul_assign(&mut inner, &a_val);
                field_ops::add_assign(&mut val, &inner);
            }
            let mut lt = unsafe { evals.get_unchecked(14usize) }[j];
            field_ops::mul_assign_by_base(
                &mut lt,
                &BabyBearField::from_reduced_raw_repr(1744830467u32),
            );
            ...
            let val = {
                let mut result: BabyBearExt4 = BabyBearExt4::ZERO;
                {
                    const CK_CONST: [(u32, usize); 4usize] = [
                        (2013003793u32, 4usize),
                        (262144u32, 5usize),
                        (1509916673u32, 6usize),
                        (2013233153u32, 7usize),
                    ];
                    let mut _i: usize = 0;
                    while _i < 4usize {
...
unsafe fn layer_1_final_step_accumulator(
    ...
    }
    {
        const SIMPLE_GATES: [(usize, [usize; 4]); 11usize] = [
            ...
            (7usize, [13usize, 14usize, 15usize, 0usize]),
            (1usize, [7usize, 0usize, 0usize, 0usize]),
            (7usize, [22usize, 23usize, 24usize, 0usize]),
            (8usize, [20usize, 21usize, 18usize, 19usize]),
            (1usize, [16usize, 0usize, 0usize, 0usize]),
            (1usize, [17usize, 0usize, 0usize, 0usize]),
