...
fn check_virtual_setup_inits_and_teardowns<E: ErrorCreator>(
    ...
    unsafe {
        ...
        if low_eval != *state.prev_claims.get_unchecked(32usize) {
            return Err(E::gkr_virtual_setup_eval_mismatch(32usize));
        }
        if high_eval != *state.prev_claims.get_unchecked(33usize) {
            return Err(E::gkr_virtual_setup_eval_mismatch(33usize));
        }
    }
    Ok(())
}
...
pub(crate) fn verify_gkr<I: NonDeterminismSource<BabyBearField>, E: ErrorCreator>(
    ...
    unsafe {
        ...
        const DIM_REDUCE_INDICES_3: [usize; 2usize] = [0usize, 1usize];
        ...
        verifier_common::stats::log("GKR COMPRESSION INIT");
        {
            ...
            let data_words = 2usize * 2 * EXT_DEGREE;
            {
                let mut i = 0;
                while i < data_words {
            ...
            {
                let evals: &[[BabyBearExt4; 2]] = eval_buf.data_as(2usize);
        ...
        }
        {
            ...
            }
            {
                ...
                let f = dim_reducing_final_step_accumulator(
                    evals,
                    state.batching_challenge,
                    &DIM_REDUCE_INDICES_3,
                );
                verify_final_step_check::<E>(f, final_eq_prefactor, final_claim, 3usize)?;
            }
            ts.commit(&mut eval_buf, data_words);
            let mut draw_buf = LazyVec::<BabyBearExt4, 2>::new();
            unsafe {
            ...
            state.prev_point_len = fc_len;
            #[cfg(feature = "verifier_stats")]
            verifier_common::stats::log("GKR COMPRESSION LAYER 3");
        }
        {
            ...
            let fc_len = 24usize;
            const NUM_AT_POINT_EVALS: usize = 4usize;
        ...
        }
        {
            ...
            let fc_len = 24usize;
            const NUM_AT_POINT_EVALS: usize = 34usize;
            let data_words = NUM_AT_POINT_EVALS * EXT_DEGREE;
            {
                let mut i = 0;
                while i < data_words {
