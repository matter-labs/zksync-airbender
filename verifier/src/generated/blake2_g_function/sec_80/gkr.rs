...
pub(crate) fn verify_gkr<I: NonDeterminismSource<BabyBearField>, E: ErrorCreator>(
    ...
    unsafe {
        ...
        }
        {
            ...
            {
                let slice: &[BabyBearExt4] = unsafe { eval_buf.data_as(123usize + 57usize) };
                let mut k = 123usize;
                while k < 123usize + 57usize {
                    extra_evals.push(unsafe { *slice.get_unchecked(k) });
                    k += 1;
                }
            }
            ts.commit(&mut eval_buf, data_words + 57usize * EXT_DEGREE);
            let next_batching = draw_single_field_el(ts);
