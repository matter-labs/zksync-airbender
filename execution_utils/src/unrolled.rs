...
impl UnrolledProgramSetup {
    ...
    pub fn continue_recursion_chain(
        ...
        // Mirror the in-circuit rule: if the chain already ends with these end params
        // (the preimage's second half), carry it through instead of extending it.
        if &previous_step_chain_preimage[8..] == &end_params[..] {
            // do not repeat
