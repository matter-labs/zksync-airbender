...
//! ([`crate::fwd::compile`]'s `place`), lifted out of that module verbatim so the
    ...
    V: Copy + Eq + Hash,
{
    ...
    *nodes -= 1;
    ...
    let mut cand: Vec<usize> =
        (0..n_quads).filter(|&q| quads[q].iter().all(|o| !r.overlaps(o))).collect();
    ...
    W: Fn(V) -> PackWidth,
{
    ...
    let (mut out, quads) = greedy_color_quads(&quad_values, ranges, n_quads)
        .ok_or(PackFailure::QuadDemandExceedsBudget { quads_needed: n_quads + 1 })?;
