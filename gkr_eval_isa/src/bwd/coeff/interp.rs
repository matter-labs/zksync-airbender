...
//!
//! Three of them, over one [`CoeffResolver`]:
//!
...
//!     (`lean` module). It has no cell file and no residency at all; what it adds
//!     instead is SEGMENTATION — the `K` per-warp term lists and the rule that
//!     exactly one of their partials carries `c_init`. The gate is again identity
//!     with the semantic interpreter, now at every `K`.
//!
