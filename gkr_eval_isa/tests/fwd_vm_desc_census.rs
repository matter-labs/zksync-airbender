...
//!   n_arg_challenge   — schedule-time-known challenges (inline `arg_challenge[..]`)
//!   n_const_challenge — runtime challenges (Task-8 `__constant__` bank)
...
const ARG_CHALLENGE_CAP: usize = 12; // hard lowering error on overflow (corpus max 7)
const CONST_CHALLENGE_CAP: usize = 8; // Task-8 __constant__ bank; hard error on overflow (corpus max 1)
...
struct Maxima {
    program_lanes: usize,
    n_consts: usize,
    n_arg_challenge: usize,
    n_const_challenge: usize,
    n_descs: usize,
    where_lanes: String,
}

fn challenge_bank_len(ctx: &gkr_eval_isa::fwd::context::DagForwardContext, sub: LdcSub) -> usize {
    let mut idx = 0u16;
    while ctx.challenges.get(sub, idx).is_some() {
        idx += 1;
    }
    idx as usize
}
...
fn fwd_vm_desc_corpus_census() {
    ...
    for name in FIXTURES {
        ...
        for (li, layer) in compiled.layers.iter().enumerate() {
            ...
            let n_arg = challenge_bank_len(&layer.ctx, LdcSub::ArgChallenge);
            let n_cch = challenge_bank_len(&layer.ctx, LdcSub::ConstChallenge);
            ...
            m.n_arg_challenge = m.n_arg_challenge.max(n_arg);
            m.n_const_challenge = m.n_const_challenge.max(n_cch);
    ...
    println!(
        "n_arg_challenge   max = {:>6}  (cap {ARG_CHALLENGE_CAP})",
        m.n_arg_challenge
    );
    println!(
        "n_const_challenge max = {:>6}  (cap {CONST_CHALLENGE_CAP})",
        m.n_const_challenge
    );
    ...
    assert!(
        m.n_arg_challenge <= ARG_CHALLENGE_CAP,
        "n_arg_challenge {} > cap",
        m.n_arg_challenge
    );
    assert!(
        m.n_const_challenge <= CONST_CHALLENGE_CAP,
        "n_const_challenge {} > cap",
        m.n_const_challenge
    );
