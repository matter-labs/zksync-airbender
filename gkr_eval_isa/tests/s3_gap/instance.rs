...
pub enum NodeKind {
    ...
    Read,
    /// `SourceKind::Prior` — real DRAM access (avoidable re-read).
    Prior,
...
pub struct OracleNode {
    ...
    pub width: u8,
    /// True iff the node incurs real DRAM traffic (Read or Prior).
    pub real_dram: bool,
...
pub struct OracleInstance {
    pub budget: usize,
    /// Remapped top-expr ids in original root visitation order.
    pub roots: Vec<u32>,
...
pub fn extract_instance(
    ...
    for &old_eid in &topo_order {
        ...
        let (kind, children) = match &layer.exprs[old_eid as usize] {
            Expr::Source(sid) => {
                ...
                let kind = match &layer.sources[sid.0 as usize].kind {
                    ...
                    SourceKind::Prior { .. } => NodeKind::Prior,
            ...
            Expr::Add(ch) => {
                let remapped: Vec<u32> = ch.iter().map(|c| remap[&c.0]).collect();
        ...
        let f = expr_operand_field(layer, ExprId(old_eid), cross);
        let width = if f == OperandField::Ext { 4 } else { 1 };
        let real_dram = matches!(kind, NodeKind::Read | NodeKind::Prior);
    ...
    OracleInstance { budget, roots, nodes }
}
...
mod tests {
    ...
    fn extract_topo_orders_children_before_parents_and_flags_dram() {
        ...
        // real_dram is exactly Read|Prior...
        let dram: Vec<_> = inst.nodes.iter().filter(|n| n.real_dram).map(|n| n.kind).collect();
        assert!(dram.iter().all(|k| matches!(k, NodeKind::Read | NodeKind::Prior)));
        // ...and the Prior must actually be PRESENT & flagged
        assert!(
            inst.nodes.iter().any(|n| matches!(n.kind, NodeKind::Prior) && n.real_dram),
            "the Prior node must exist and be real_dram (included here, excluded from D)"
        );
        // ext Read has width 4, base Read width 1
        assert!(inst.nodes.iter().any(|n| matches!(n.kind, NodeKind::Read) && n.width == 4));
        assert!(inst.nodes.iter().any(|n| matches!(n.kind, NodeKind::Read) && n.width == 1));
    }
    ...
    fn distinct_live_values_counts_carry_across_roots() {
        ...
        let layer = DagLayer {
            ...
            roots: vec![
                Root::Output { expr: ExprId(5), sink: SinkId(0) },
                Root::Output { expr: ExprId(6), sink: SinkId(1) },
                Root::Output { expr: ExprId(7), sink: SinkId(2) },
            ],
            sinks: vec![
    ...
    fn resolution_pruned_expr_yields_special_terminal() {
        ...
        use std::collections::BTreeMap;
        use cs::gkr_compiler::dag_ir::{
            BatchingOrder, Expr, ExprId, RangeWidth, ResolutionStrategy,
            Root, SinkId, SinkInfo, SinkKind, SourceId, SourceInfo, SourceKind,
        };
        ...
        let layer = DagLayer {
            ...
            roots: vec![Root::Output { expr: ExprId(1), sink: SinkId(0) }],
            sinks: vec![SinkInfo {
        ...
        assert_eq!(inst.nodes.len(), 1, "only the Special terminal node should be emitted");
        ...
        assert!(node.children.is_empty(), "Special terminal must have children: vec![]");
