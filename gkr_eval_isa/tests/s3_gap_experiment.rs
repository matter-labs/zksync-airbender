...
mod s3_gap;
// `mod s3_planner` DELETED (Task 6): the metaheuristic/forkset/inner-dp prototypes were
...
// DELETED (Task 6): the M1 inner-DP planner gates (`m1_planner_is_deterministic`,
// `m1_planner_matches_oracle_e_across_corpus`, `m1_uniform_binding_c_is_exact_handbuilt`),
// `replay_plan_matches_score_and_reproduces_residency`, and
// `peak_with_cached_treats_cached_mul_as_leaf_with_own_production_cone` tested the
// `s3_planner` replay/forkset simulation internals, which are deleted wholesale —
// the scorer now compiles candidates for real (`schedule_search::scorer::score`).
...
// DELETED (Task 6): `atom_root_ids`/`bridge_order`/`invert_remap` +
// `order_bridge_binding_holds` bridged OracleInstance occurrence indices to production
// `RootId`s; the schedule search now decodes `Vec<RootId>` directly from cs's DagLayer
// (`schedule_search::decode::decode_order` over `structure::relation_units`), so there
// is no occurrence/RootId bridge left to bind.
...
// (`schedule_search::producer::produce_circuit_schedule`) — Task 6.

// DELETED (Task 6): the Task-5 11-fixture cross-check
// (`site_enumeration_matches_oracle_instance_derivation` + its `NormSite`/`SiteWalkCfg`
// normalization machinery) validated the new cs-backed site domain against the old
// OracleInstance-based `enumerate_cache_priority_sites`/`classify_values` — both sides
// of that differential are gone with the prototype: cs's `enumerate_site_domain` is the
// single source of truth (validator check b) and the compile-based scorer consumes it
// directly. The synthetic query-edge pin below survives (prototype-free).
