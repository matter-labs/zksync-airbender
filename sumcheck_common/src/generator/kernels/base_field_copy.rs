...
pub(crate) fn generate_compute_fns<F: PrimeField, E: FieldExtension<F> + Field>(
    ...
    let input_scratch_to_use = pos_state.get(&input).expect("data").cache_pos;
    ...
    let compute_fn = quote! {
        ...
        pub fn #compute_fn_id<F: PrimeField, E: FieldExtension<F> + Field, S: SumcheckRoundSource<F, E>, const EXPLICIT_FORM: bool>(
            ...
            if EXPLICIT_FORM {
                let c0 = base_field_scratch[#input_scratch_to_use][0].mul_by_ext(&sumcheck_challenges[#challenge_to_use], base_repr_ctx);
                let c1 = base_field_scratch[#input_scratch_to_use][1].mul_by_ext(&sumcheck_challenges[#challenge_to_use], base_repr_ctx);
