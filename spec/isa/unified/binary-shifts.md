...
- `ZimopIXorRot rd, rs1, r` for `r ∈ {16, 12, 8, 7}`
...
- `rd_old ∈ u32` is the pre-cycle value of the destination register
- `imm12 ∈ u12` is the encoded immediate
- `r ∈ u5` is the encoded `ZimopIXorRot` rotation
...
- `ror32(x, r)` cyclically rotates the 32-bit word `x` right by `r` bits
...
- **ASM-UBSHIFT-002* — Xor-rotate decoder boundary.** For an active `ZimopIXorRot`
  row, the unified decoder authenticates `rs1`, selects the pre-cycle value of `rd` as
  the second source, and authenticates `r ∈ {16, 12, 8, 7}`.
...
- **`execute = 1`**
  ...
  - **[`REL-UBSHIFT-002*`] Custom xor-rotate assignment**
    - **`op = ZimopIXorRot`, `r ∈ {16, 12, 8, 7}`**
      `rd ← ror32(rs1 ^ rd_old, r)`
...
- **GAP-UBSHIFT-001 — Custom xor-rotate adoption.** Adopt or replace the
  `ZimopIXorRot` operand relation and the exact rotation set `{16, 12, 8, 7}`. The
  current relation is independently enforced by decoder, lookup-table, reconstruction,
  and decoder-test code, but has no adopted ISA or project-design reference.
...
| `ASM-UBSHIFT-002` | provisional | active `ZimopIXorRot` row | `external:DEC`; `REQ-UNIFIED-001`; `GAP-UBSHIFT-001` | checked | xor-rotate preprocessing, decoder dispatch, and decoder rotation-map test at `dfb1b2a8a` | `symbol:riscv_transpiler/src/ir/simple_instruction_set.rs#preprocess_bytecode`; `symbol:cs/src/gkr_circuits/unified_reduced_machine/decoder.rs#UnifiedReducedMachineDecoder::define_decoder_subspace`; `check:cs/src/gkr_circuits/unified_reduced_machine/decoder.rs#tests::xor_rot_unified_only` |
...
| `GAP-UBSHIFT-001` | open | — | affects `ASM-UBSHIFT-002`, `REL-UBSHIFT-002`; owner: human | — | no adopted ISA or project-design reference for `ZimopIXorRot` | — |
