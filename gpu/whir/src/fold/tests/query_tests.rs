        ...
        .map(|host| {
            (0..columns.len())
                .map(|column| {
                    ...
                    (0..rows)
                        .map(|row| host[start + bitreverse_index(row, log_domain_size)])
                        .collect::<Vec<_>>()
                })
                .collect::<Vec<_>>()
        })
        .collect::<Vec<_>>();
    let source_refs_storage = source_storage
        .iter()
        .map(|coset| coset.iter().map(|column| &column[..]).collect::<Vec<_>>())
        .collect::<Vec<_>>();
    let source_refs = source_refs_storage
        .iter()
