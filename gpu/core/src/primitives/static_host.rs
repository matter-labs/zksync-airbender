...
pub type StaticPinnedBox<T> = Box<[T], ConcurrentStaticHostAllocator>;

pub fn alloc_static_pinned_box_uninit<T>(len: usize) -> CudaResult<StaticPinnedBox<T>> {
...
pub fn alloc_static_pinned_box_from_slice<T: Copy>(values: &[T]) -> CudaResult<StaticPinnedBox<T>> {
