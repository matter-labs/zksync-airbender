...
mod tests {
    ...
    use crate::forward::compile::compile_circuit;
    use crate::forward::context::CompiledLayer;
    use crate::forward::source::SpecialStrategy;
    ...
    fn compile_layer0(name: &str, stem: &str) -> Option<(CompiledLayer, usize)> {
        ...
        let compiled = compile_circuit(&dag, &sched).expect("compile_circuit failed");
    ...
    fn count_global_reads(instr: &crate::forward::isa::Instr) -> usize {
    ...
    fn add_sub_layer0_dram_reads_counted() {
        ...
        let alias_globals = compiled
            ...
            .filter(|(_, out)| {
                matches!(
                    out,
                    crate::forward::context::RootOutput::Alias(
                        crate::forward::isa::OperandLine::LogicalGlobal { .. }
                    )
                )
            })
            .count();
