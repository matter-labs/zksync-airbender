...
struct VirtualLower<'a> {
    ...
    top_bits_descs: HashMap<gkr_eval_ir::InitsAndTeardownsTopBitsRef, u16>,
...
impl<'a> VirtualLower<'a> {
    ...
    fn intern_top_bits(
        ...
        if let Some(&desc) = self.top_bits_descs.get(&reference) {
            return desc;
        }
        let desc = self
            .ctx
            .specials
            .push(super::super::source::SpecialDescriptor {
                strategy: super::super::source::SpecialStrategy::InitsAndTeardownsTopBits {
                    reference,
                },
                origin_expr,
            });
        self.top_bits_descs.insert(reference, desc);
        desc
    }
    ...
    fn source_to_vop(
        ...
        match &layer.sources[src_id.0 as usize].kind {
            ...
            SourceKind::InitsAndTeardownsTopBits { reference } => {
                let desc = self.intern_top_bits(*reference, expr_id);
                Ok(VirtualOp::Special { desc })
            }
...
> {
    ...
    for (p, &rid) in atom_order.iter().enumerate() {
        ...
        match action {
            ...
            Some(ForwardAction::CopyAlias { src, .. }) => {
                if !st.exposed.contains(&rid) {
                    ...
                    let field = super::read_place_operand_field(&src, &st.cross, sink_field);
                    let (slot, col) = st.ctx.backings.read_slot_col(&src, field)?;
...
fn sorted_real(defined: &HashSet<ExprId>) -> Vec<ExprId> {
    let mut values: Vec<_> = defined
        .iter()
        .copied()
        .filter(|expr| expr.0 < INTERNAL_BASE)
        .collect();
    values.sort_by_key(|expr| expr.0);
    values
}
