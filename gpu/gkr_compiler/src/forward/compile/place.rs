...
use crate::forward::isa::{LdcSub, OperandField, Sign};
...
mod tests {
    ...
    use crate::forward::isa::{OperandField as F, Sign};
    use gkr_eval_ir::ExprId;
    use std::collections::HashMap;
    ...
    fn load(n: u32, field: F, col: u16) -> VirtualInstr {
        VirtualInstr {
            ...
            field,
            defines: Some(v(n)),
            reads: vec![VirtualOp::Global { slot: 0, col }],
            sign: Sign::Plus,
            is_dram_read: true,
        }
    }
    ...
    fn assert_ext_base_disjoint(p: &Placement, widths: &HashMap<ExprId, F>) {
        ...
        for (&i, vals) in &by_instr {
            let ext_quads: Vec<u16> = vals
                .iter()
                .filter(|(val, _)| widths[val] == F::Ext)
                .map(|&(_, c)| c / 4)
                .collect();
            for &(val, c) in vals {
        ...
        assert!(
            p.moves.is_empty(),
            "two-phase placement is relocation-free by construction"
        );
    }
    ...
    fn ext_values_get_4_aligned_nonoverlapping_cells() {
        ...
        assert!(
            (c1 as i32 - c2 as i32).abs() >= 4,
            "overlapping Ext lifetimes must not share a 4-cell span"
        );
    ...
    fn oversized_ext_set_is_rejected() {
        ...
        assert!(
            plan_placement(&input).is_err(),
            "3 concurrent Ext do not fit in 2 quads"
        );
    }
    ...
    fn realized_live_set_matches_resident_after() {
        ...
        assert!(
            p.cell_of.contains_key(&(last, v(2))),
            "resident_after value 2 must hold a cell"
        );
        assert!(
            !p.cell_of.contains_key(&(last, v(1))),
            "dropped value 1 must NOT hold a cell at step end"
        );
        assert!(
            p.moves.is_empty(),
            "an implicit drop frees a cell with no move"
        );
    }
    ...
    fn former_compaction_case_needs_zero_relocations() {
        ...
        let mut instrs: Vec<VirtualInstr> = (1..=11u32)
            .map(|n| load(n, F::Base, (n - 1) as u16))
            .collect();
        ...
        assert!(
            p.moves.is_empty(),
            "two-phase resolves this with zero relocations"
        );
    ...
    fn ranges_of(
        specs: &[(u32, usize, usize, F)],
    ) -> (HashMap<ExprId, LiveRange>, HashMap<ExprId, F>) {
    ...
    fn oracle_feasible(
        ...
        let mut exts: Vec<ExprId> = ranges
            .keys()
            .copied()
            .filter(|v| widths[v] == F::Ext)
            .collect();
        let mut bases: Vec<ExprId> = ranges
            .keys()
            .copied()
            .filter(|v| widths[v] == F::Base)
            .collect();
    ...
    fn base_blind_greedy_stranding_is_recovered_by_search() {
        ...
        assert!(
            oracle_feasible(&ranges, &widths, 8),
