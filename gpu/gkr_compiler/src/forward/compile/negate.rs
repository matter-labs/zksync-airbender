...
pub fn canonicalize_product(factors: &[OperandLine]) -> (bool, Vec<OperandLine>) {
    let neg_one_count = factors.iter().filter(|&&op| is_neg_one(op)).count();
...
mod tests {
    ...
    fn product_with_neg_one_splits() {
        let (neg, rest) = canonicalize_product(&[
            OperandLine::Smem { cell: 0 },
            neg_one(),
            OperandLine::Smem { cell: 4 },
        ]);
        assert!(neg);
        assert_eq!(
            rest,
            vec![OperandLine::Smem { cell: 0 }, OperandLine::Smem { cell: 4 }]
        );
    }
    #[test]
    ...
    #[test]
    fn zero_operand_rejected() {
        assert!(assert_no_zero_operand(&[zero()]).is_err());
    }
}
