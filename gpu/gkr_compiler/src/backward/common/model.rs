...
use gkr_eval_ir::{
    ChallengeKey, ChallengePower, ChallengeRef, ChallengeResolver, ExprId, FieldKind,
    PermutationSlot, ReadPlace, RootId, SinkKind, VirtualSetupKind,
};
...
/// in [`CoeffLayer::coefficients`]. Every other id addresses
...
/// Terms sharing a challenge core. Members
...
/// read.
pub(crate) fn source_order_key(origin: &OriginLeaf) -> (u8, u8, usize, usize) {
    match origin {
...
// ── Normalized coefficient recipes ───────────────────────────────────────────

/// A challenge factor with a total order, in canonical spelling.
///
/// `ChallengeRef` is deliberately not `Ord`, so this wrapper projects it to a
/// stable tuple.
///
...
impl Ord for CoeffChallenge {
    fn cmp(&self, other: &Self) -> Ordering {
        challenge_order_key(&self.0).cmp(&challenge_order_key(&other.0))
    }
}
...
fn challenge_order_key(reference: &ChallengeRef) -> (u8, u8, u8, u32) {
    let (key, sub) = match &reference.key {
        ChallengeKey::LookupAdditive => (0, 0),
        ...
        ChallengeKey::ClaimBatching => (4, 0),
    };
    let (power, exponent) = match reference.power {
        ChallengePower::One => (0, 0),
        ChallengePower::Static(exponent) => (1, exponent),
    };
    (key, sub, power, exponent)
}

/// A canonical scalar times a sorted multiset of challenge factors.
///
...
///
/// Repeated factors remain separate because some challenge resolvers ignore the
/// encoded power.
#[derive(Clone, Debug, PartialEq, Eq, Hash)]
pub struct CoeffProduct {
    pub scalar: u32,
    pub challenges: Vec<CoeffChallenge>,
    pub inits_and_teardowns_top_bits: Vec<gkr_eval_ir::InitsAndTeardownsTopBitsRef>,
}
        ...
        self.challenges
            ...
            .then_with(|| {
                self.inits_and_teardowns_top_bits
                    .cmp(&other.inits_and_teardowns_top_bits)
            })
...
/// challenges)`.
///
/// Normalization is total and order-free: scalar constants, batch powers and
...
impl NormalizedCoefficientRecipe {
    ...
    pub fn inits_and_teardowns_top_bits(
        reference: gkr_eval_ir::InitsAndTeardownsTopBitsRef,
    ) -> Self {
        Self::from_terms(vec![CoeffProduct {
            scalar: 1,
            challenges: Vec::new(),
            inits_and_teardowns_top_bits: vec![reference],
        }])
    }
    ...
    pub fn from_terms(mut terms: Vec<CoeffProduct>) -> Self {
        ...
        let mut merged: std::collections::BTreeMap<
            (
                Vec<CoeffChallenge>,
                Vec<gkr_eval_ir::InitsAndTeardownsTopBitsRef>,
            ),
            Bf,
        ...
        for t in terms.drain(..) {
            ...
            let mut challenges: Vec<CoeffChallenge> = t
                ...
                .collect();
            challenges.sort();
            let mut top_bits = t.inits_and_teardowns_top_bits;
            top_bits.sort();
            let slot = merged.entry((challenges, top_bits)).or_insert(Bf::ZERO);
        ...
        let terms = merged
            ...
            .map(
                |((challenges, inits_and_teardowns_top_bits), v)| CoeffProduct {
                    scalar: v.as_u32_reduced(),
                    challenges,
    ...
    pub fn mul(&self, other: &Self) -> Self {
        ...
        for a in &self.terms {
            for b in &other.terms {
                ...
                let mut inits_and_teardowns_top_bits = a.inits_and_teardowns_top_bits.clone();
                inits_and_teardowns_top_bits.extend(b.inits_and_teardowns_top_bits.iter().copied());
    ...
    pub fn evaluate(&self, challenge: &dyn ChallengeResolver) -> Ext {
        ...
        for term in &self.terms {
            let mut product =
                <Ext as FieldExtension<Bf>>::from_base(Bf::from_u32_with_reduction(term.scalar));
            for reference in &term.challenges {
                product.mul_assign(&challenge.challenge(&reference.0));
            }
            for reference in &term.inits_and_teardowns_top_bits {
                let value = (reference.set_index as u32)
                    .checked_shl(reference.shift)
                    .unwrap_or(0);
                product.mul_assign(&<Ext as FieldExtension<Bf>>::from_base(
...
pub enum CoeffTerm {
    ...
    DualProduct {
        id: TermId,
        coefficient: CoefficientRecipeId,
        /// A dual factor consumes both projections of each source.
        lhs: SourceId,
        rhs: SourceId,
    },
}
...
pub struct CoeffLayer {
    ...
    /// Empty until the grouping transform runs;
...
pub enum CoeffError {
    ...
    ///
    /// The program does not fit the coefficient encoding.
    ...
    /// Semantic grouping analysis can use the full non-reserved u16 immediate-id
    /// space even when the production wire's smaller table would overflow. This
    /// is the only remaining representational fence on analysis materialization.
    AnalysisImmediateIdOverflow {
        len: usize,
    },
