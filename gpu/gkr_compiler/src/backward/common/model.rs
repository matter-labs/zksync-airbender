...
use gkr_eval_ir::{
    Bf, ChallengeKey, ChallengePower, ChallengeRef, ChallengeResolver, ExprId, Ext, FieldKind,
...
impl NormalizedCoefficientRecipe {
    ...
    pub fn evaluate(&self, challenge: &dyn ChallengeResolver) -> Ext {
        ...
        for t in &self.terms {
            ...
            for reference in &t.inits_and_teardowns_top_bits {
                let value = (reference.set_index as u32)
                    .checked_shl(reference.shift)
                    .unwrap_or(0);
                prod.mul_assign(&<Ext as FieldExtension<Bf>>::from_base(
