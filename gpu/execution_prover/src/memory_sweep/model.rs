...
pub fn generate_policy(input: impl Read, output: impl Write) -> Result<(), SweepModelError> {
    ...
    writeln!(output, "use gpu_circuit_prover::proof::memory_policy::{{ProofMemoryPolicy, OpeningStrategy, WitnessMemoryPolicy, WitnessCommitmentStrategy, WitnessPostCommitStorage, WitnessOpeningStrategy}};")?;
...
pub fn stable_name(policy: MemoryPolicy) -> String {
    let (setup, memory, commitment, post_commitment, opening) = policy_fields(policy);
    format!("setup_{setup}-memory_{memory}-commitment_{commitment}-post_commitment_{post_commitment}-opening_{opening}")
}
...
fn policy_expression(policy: MemoryPolicy) -> String {
    ...
    format!("ProofMemoryPolicy {{ setup: OpeningStrategy::{:?}, memory: OpeningStrategy::{:?}, witness: WitnessMemoryPolicy {{ commitment: WitnessCommitmentStrategy::{:?}, post_commitment: WitnessPostCommitStorage::{:?}, opening: {opening} }} }}", policy.setup, policy.memory, policy.witness.commitment, policy.witness.post_commitment)
}
