...
pub fn generate_policy(input: impl Read, output: impl Write) -> Result<(), Box<dyn Error>> {
    ...
    writeln!(output, "use gpu_circuit_prover::proof::memory_policy::{{ProofMemoryPolicy, GkrMemoryPolicy, OpeningStrategy, WitnessMemoryPolicy, WitnessCommitmentStrategy, WitnessPostCommitStorage, WitnessOpeningStrategy}};")?;
