...
unsafe fn copy_to_symbols(
    ...
    memcpy_to_symbol(&ab_inv_cmem_twiddles_fine, &inv_cmem_twiddles_fine)?;
    memcpy_to_symbol(
...
impl DeviceContext {
    pub fn create(powers_of_w_coarse_log_count: u32) -> CudaResult<Self> {
        ...
        // `coarse_log_count + fine_log_count == OMEGA_LOG_ORDER` so that the
        // 2-layer table `get_power_from_layers` can decode exponents over the
        // full `[0, 2^OMEGA_LOG_ORDER)` range — necessary for the coset-shift
        // lookup `bitrev(row, log_n) * coset_factor_power` to be correct for
        // any reachable `log_n + log_lde_factor <= OMEGA_LOG_ORDER`.
        ...
        let fine_base_inv = fine_base
            .inverse()
            .expect("BabyBear inverse fine twiddle generator must exist");
