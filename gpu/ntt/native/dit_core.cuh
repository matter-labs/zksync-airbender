...
template <unsigned S, unsigned LOG_M, unsigned LOG_VPT, unsigned LOG_TBL> DEVICE_FORCEINLINE void apply_local_stages_pt(bf w[], const bf *tw, unsigned tw_row) {
...
template <unsigned LOG_M, unsigned LANES, unsigned LOG_TBL, unsigned LOG_VPT, bool SKIP_LAST_TW>
DEVICE_FORCEINLINE void dit_phase(const bf *tw, unsigned tw_base, unsigned tw_row, unsigned lane, bf w[]) {
  constexpr unsigned VPT = 1u << LOG_VPT;
  ...
  if constexpr (LOG_M > LOG_VPT) {
    ...
    const unsigned subwarp_mask = (LANES >= 32u) ? 0xFFFFFFFFu : (((1u << (LANES & 31u)) - 1u) << subwarp_base);
...
#pragma unroll
      for (unsigned j = 0; j < HALF; ++j) {
        ...
        const unsigned raw = __shfl_xor_sync(subwarp_mask, bf::into_raw_u32(send), lane_mask, LANES);
