...
///
/// Uses the same `hashed_fibonacci` workload as that stagewise test (known to
/// produce non-empty RAM touches). When `compute_cpu_reference` is false the
...
pub(super) fn prepare_inits_and_teardowns_proof_fixture(
    ...
    // --- VM run (same workload as the stagewise standalone i/t test) ---
    ...
    // --- challenges / config (identical to the stagewise standalone test) ---
...
fn standalone_inits_and_teardowns_gpu_workflow_matches_cpu() {
    ...
    let canonical_top_bits =
        crate::proof::canonical_inits_and_teardowns_top_bits(&compiled_circuit);
    let mut bundle = crate::proof::inputs::GpuGKRProofTransfer::<'_, Global>::new(
        None,
        None,
