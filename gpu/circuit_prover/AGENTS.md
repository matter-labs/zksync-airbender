...
- **Feature-gated test suites: `--features bench` is required, and its absence is silent.** The backward segmented-lean-VM GPU suite (`prover::gkr::backward::vm::{seg_gpu_tests, seg_report, seg_compile}`) is `#[cfg(all(test, feature = "bench"))]`, so a plain `cargo test -p gpu_circuit_prover` compiles none of it and **exits 0 having run none of those gates**. Its always-compiled `bwd_coeff_gating_notice` test prints that fact in the default run's output; export `AB_REQUIRE_BENCH_GATES=1` to turn a `bench`-less run into a failure when "the GPU gates ran" is the claim being made. Its GPU gates are additionally `#[ignore]`d, so they need `--ignored` on top of the feature — and libtest exits 0 when a filter matches nothing, so always report the tests-run count, never a bare exit status. The `seg_report::bwd_seg_corpus_*` sweeps are long runs; the per-task correctness gate is `seg_gpu_tests::` with `--ignored`.
