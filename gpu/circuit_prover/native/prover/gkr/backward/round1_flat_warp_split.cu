...
// Device-pointer coefficient variant of the round 1 warp-split kernel.
// Identical algebra; coefficients are read from the `coefficients` device
// buffer instead of the `ab_gkr_flat_coefficients` __constant__ symbol.
// Selected when the continuation coefficient count exceeds FLAT_CONST_MAX.
EXTERN __launch_bounds__(128, 8) __global__
    void ab_gkr_main_round1_flat_devptr_compact_unified_compact_e4_kernel(const __grid_constant__ flat_round1_unified_desc_compact desc,
...
// Device-pointer TERMS variant of the round 1 warp-split kernel. The three
// large arrays (`terms`, `tile_term_offsets`, `tile_fold_offsets`) move to a
// `flat_term_tables` bundle in device memory, and the `_devptr` descriptor
// carries only the remaining (small) fields. Coefficients also come from a
// device buffer, since a term overflow implies a coefficient overflow. Selected
// when the inline `flat_round1_unified_desc_compact` would exceed the 32 KB
// __grid_constant__ cap. Identical algebra to the constant variant.
EXTERN __launch_bounds__(128, 8) __global__ void ab_gkr_main_round1_flat_devptr_terms_compact_unified_compact_e4_kernel(
