...
The second form prints the SEG line the committed oracle implies and exits — `r7_gates.sh`
...
TAGS = ("SEG-ANCHOR", "SEG-SMEM", "SEG-GMEM")
TAG_RE = "|".join(TAGS)
SCHED = re.compile(
...
def load(path, index, oracles):
    ...
    if sched["tag"] != want_tag or sched["order"] != want_order:
        die(f"{where}: this position is preregistered as {want_tag} at "
            ...
            f"order={sched['order']} — the eight logs are positional, so a mis-ordered set "
            f"would be summarized under the wrong pairing")
    ...
    deals = want_tag in ("SEG-SMEM", "SEG-GMEM")
    if not deals:
        if seg is not None:
...
def session(paths):
    """The eight processes plus the cross-log gates: one experiment, one dealt plan per term
    order, one build."""
    if len(paths) != len(POSITIONS):
        die(f"r7_table expects exactly {len(POSITIONS)} logs in session order "
            f"({' '.join(p[0] for p in POSITIONS)}); got {len(paths)} — every pairing here is "
            f"POSITIONAL, so a short or long set has no defined positions")
    oracles = oracle()
    procs = [load(p, i, oracles) for i, p in enumerate(paths)]
...
def paired(p, a, b):
    """The paired per-round contrast `a - b` on eval+finalize inside ONE process, with its
    sign-stability count. Reported, never turned into a verdict (R7 preregisters none)."""
    d = [x - y for x, y in zip(p["tot"][a], p["tot"][b])]
...
def inventory_table(procs):
    print("### Session inventory — eight positional processes\n")
