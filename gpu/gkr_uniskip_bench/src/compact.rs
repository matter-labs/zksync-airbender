...
/// four coordinate hyperplanes of the 4-bit tap index. `perm` is the GF(2)-linear map
/// with column images `[1, 2, 5, 14]`, i.e.
/// `(t0, t1, t2, t3) -> (t0^t2, t1^t3, t2^t3, t3)`; it is the (unique up to the search's
/// tie-break) linear permutation whose low 3 bits stay a bijection on all four
/// hyperplanes AND whose low 2 bits stay one on every 4-slot round window. The identity
/// satisfies neither — `{0,1,2,3,8,9,10,11}` collides pairwise mod 8 — and that cost
/// 79 % excess shared wavefronts in the first build of this mode.
pub const fn bank_perm(tap: usize) -> usize {
    (tap & 1) ^ ((tap >> 2) & 1)
        | ((((tap >> 1) & 1) ^ ((tap >> 3) & 1)) << 1)
        | ((((tap >> 2) & 1) ^ ((tap >> 3) & 1)) << 2)
        | (((tap >> 3) & 1) << 3)
...
pub const fn staging_offset(groups: usize, group: usize, tap: usize) -> usize {
    bank_perm(tap) * groups + group
}
...
fn ordered_slots(groups: usize, multiplying: Vec<usize>, plain: Vec<usize>) -> Vec<usize> {
    ...
    for mut partition in [multiplying, plain] {
        while !partition.is_empty() {
            ...
            let pick = partition
                .iter()
                .position(|&slot| !used.contains(&(bank_perm(slot) % window)))
                .unwrap_or(0);
            let slot = partition.remove(pick);
            used.push(bank_perm(slot) % window);
            out.push(slot);
        }
    }
    out
...
/// runtime value.
pub fn schedule(groups: usize) -> Vec<UniskipCompactSlot> {
    assert!(
    ...
    for phase in &PHASES {
        ...
        let order = ordered_slots(groups, multiplying, plain);
        ...
        for j in 0..items {
            ...
            out.push(UniskipCompactSlot {
                lo: staging_offset(groups, group, slot) as u16,
                hi: staging_offset(groups, group, phase.multiplied(slot)) as u16,
                tw,
            });
        }
    }
...
pub fn schedule_words(groups: usize) -> Vec<UniskipCompactSlot> {
    let mut out = schedule(groups);
    out.resize(
...
mod cpu_tests {
        ...
        for groups in [4usize, 8] {
            ...
            for (p, phase) in PHASES.iter().enumerate() {
                ...
                let unperm: Vec<usize> = (0..UNISKIP_TAPS)
                    .map(|t| (0..UNISKIP_TAPS).position(|x| bank_perm(x) == t).unwrap())
                    .collect();
                for entry in slice {
                    ...
                    assert_eq!(
                        entry.hi as usize,
                        staging_offset(groups, group, phase.multiplied(slot)),
    ...
    fn cpu_compact_bank_perm_is_hyperplane_bijective() {
        let image: std::collections::HashSet<usize> = (0..UNISKIP_TAPS).map(bank_perm).collect();
        ...
        for t in 0..UNISKIP_TAPS {
            ...
            assert_eq!(bank_perm(t), linear, "tap {t}");
        }
    }
}
