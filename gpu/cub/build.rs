...
fn main() {
    ...
    let enable_lineinfo = std::env::var_os("GPU_CUB_ENABLE_LINEINFO").is_some();
    println!("cargo:rerun-if-env-changed=GPU_CUB_ENABLE_LINEINFO");
    ...
    if is_no_cuda() {
        ...
        config.define(
            "GPU_CUB_ENABLE_LINEINFO",
            if enable_lineinfo { "ON" } else { "OFF" },
        );
        let dst = config.build();
        let gpu_cub_native_path = dst.to_str().unwrap();
        println!("cargo:rustc-link-search=native={gpu_cub_native_path}");
        println!("cargo:rustc-link-lib=static=gpu_cub_archive");
