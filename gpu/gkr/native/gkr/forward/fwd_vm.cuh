...
// (`gpu/circuit_prover/src/forward/vm/desc.rs`) with an exact-size
...
struct fwd_vm_desc {
  ...
  u32 n_arg_derived_e4;        // 1468

  // special descriptors
  u32 n_descs;            // 1472
  u32 n_const_derived_e4; // 1476: used length of the Task-8 __constant__ bank
                          // (INCLUDING the appended decoder fill slot, if any);
                          // read only under VALIDATE
  u32 fill_bank_idx;      // 1480: const-derived-e4 bank slot of the decoder fill
  ...
  u32 descs[FWD_VM_DESC_CAP]; // 1488, 1,480 B

  // geometry
  u32 count; // 2968: rows (= trace_len = mapping-arena column stride)

  // program, inline 16-bit wire lanes (gkr_eval_isa::fwd::encode)
  u16 program[FWD_VM_PROGRAM_CAP]; // 2972, 24,576 B; field bytes end at 27,548
};

static_assert(sizeof(fwd_vm_desc) == 27552, "fwd_vm_desc/FwdVmDesc ABI size drift");
...
static_assert(__builtin_offsetof(fwd_vm_desc, n_arg_derived_e4) == 1468, "n_arg_derived_e4 ABI offset drift");
static_assert(__builtin_offsetof(fwd_vm_desc, n_const_derived_e4) == 1476, "n_const_derived_e4 ABI offset drift");
