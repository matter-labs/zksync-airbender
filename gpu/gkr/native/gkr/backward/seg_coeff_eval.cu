...
EXTERN __launch_bounds__(128, 8) __global__
    void ab_gkr_bwd_seg_eval_coefficients_kernel(__grid_constant__ const bwd_seg_coeff_chunk_desc desc, const e4 *challenges, e4 *coefficients) {
  const unsigned gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid >= desc.bank_count)
    return;
  coefficients[desc.bank_first + gid] = bwd_seg_eval_coefficient(desc, gid, challenges);
}
