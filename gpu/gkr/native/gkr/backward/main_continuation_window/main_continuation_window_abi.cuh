...
// Dedicated width-three continuation-window ABI. The shared window header
// supplies address lanes, coefficient-bank access, fold weights, Eq geometry
// and procedural-source primitives.
...
constexpr u32 BWD_MAIN_CONT_WINDOW_PUBLICATION_BLOCK_THREADS = BWD_MAIN_CONT_WINDOW_BLOCK_WARPS * BWD_WINDOW_WARP_LANES;
constexpr u32 BWD_MAIN_CONT_WINDOW_PUBLICATION_LANES_PER_ROW = 4;
constexpr u32 BWD_MAIN_CONT_WINDOW_PUBLICATION_ROWS_PER_BLOCK = BWD_WINDOW_WARP_LANES / BWD_MAIN_CONT_WINDOW_PUBLICATION_LANES_PER_ROW;
...
struct alignas(16) bwd_main_cont_window_desc {
  ...
  bwd_source_window slot[BWD_MAIN_CONT_WINDOW_ADDR_SLOTS];
  u32 c_init_coeff;
...
static_assert(BWD_MAIN_CONT_WINDOW_PROGRAM_WORD_CAP == BWD_CONTINUATION_PROGRAM_WORD_CAP, "continuation program capacity drift");
static_assert(BWD_MAIN_CONT_WINDOW_MAX_SOURCES == BWD_CONTINUATION_MAX_SOURCES, "continuation source capacity drift");
static_assert(BWD_MAIN_CONT_WINDOW_ADDR_SLOTS == BWD_SOURCE_WINDOW_SLOTS, "continuation address capacity drift");
static_assert(BWD_MAIN_CONT_WINDOW_MAX_IMMEDIATES == BWD_CONTINUATION_MAX_IMMEDIATES, "continuation immediate capacity drift");
static_assert(BWD_MAIN_CONT_WINDOW_MAX_SOURCES < BWD_PROGRAM_SOURCE_NONE, "semantic SourceId collides with source-none sentinel");
static_assert(BWD_MAIN_CONT_WINDOW_ADDR_SLOTS * (1u << BWD_SOURCE_LANE_COLUMN_BITS) <= BWD_SOURCE_LANE_NONE,
              "continuation address lane collides with lane-none sentinel");
static_assert(BWD_MAIN_CONT_WINDOW_BLOCK_THREADS == BWD_MAIN_CONT_WINDOW_BLOCK_WARPS * BWD_WINDOW_WARP_LANES, "one physical warp per selector");
...
static_assert(BWD_MAIN_CONT_WINDOW_ROWS_PER_TILE == BWD_WINDOW_WARP_LANES, "continuation row tile drift");
static_assert(BWD_MAIN_CONT_WINDOW_SHAPE_DEFINED_BITS == 0x1f, "continuation shape width drift");
static_assert(BWD_CONTINUATION_CLASS_C0_LINEAR_E4 == 0, "continuation linear class drift");
static_assert(BWD_CONTINUATION_CLASS_DUAL_PRODUCT_E4 == 1, "continuation dual-product class drift");
static_assert(BWD_CONTINUATION_CLASS_GROUP_HEADER == 2, "continuation group class drift");
...
template <typename T> DEVICE_FORCEINLINE const T *bwd_main_cont_window_column(const bwd_main_cont_window_desc &desc, const u16 lane) {
  const bwd_source_window &slot = desc.slot[bwd_main_cont_window_lane_slot(lane)];
