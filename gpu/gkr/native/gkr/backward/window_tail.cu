...
#pragma unroll 1
    for (u32 round = 0; round < 3; ++round) {
      ...
      e = bwd_window_warp_sum(e);
      c = bwd_window_warp_sum(c);
      // inv(0) is zero; reconverge before consuming either lane result.
      if (lane < 2)
        inverse[lane] = e4::inv(lane == 0 ? *eq : rho[round]);
      __syncwarp();
      if (lane == 0) {
        const e4 r = bwd_window_tail_round_with_inverses(e, c, rho[round], inverse[0], inverse[1], seed, claim, eq, coeffs + 4 * round);
        challenge = r;
        challenges[round] = r;
      }
      __syncwarp();
  ...
  ::airbender::gkr::fold_active_eq_slot<BWD_WINDOW_TAIL_BLOCK_THREADS>(active_eq, eq_bits);
}
