...
// Wire and launch ABI of the window-3 sectioned backward executor.
#include "common.cuh"
...
static_assert(BWD_WINDOW_OPCODE_LINEAR_BF == BWD_R0_CLASS_C0_LINEAR_BF, "window opcode 0 must stay the lean BF linear class");
static_assert(BWD_WINDOW_OPCODE_LINEAR_E4 == BWD_R0_CLASS_C0_LINEAR_E4, "window opcode 1 must stay the lean E4 linear class");
static_assert(BWD_WINDOW_OPCODE_PRODUCT_BF_BF == BWD_R0_CLASS_C2_PRODUCT_BF_BF, "window opcode 2 must stay the lean BFxBF class");
static_assert(BWD_WINDOW_OPCODE_PRODUCT_BF_E4 == BWD_R0_CLASS_C2_PRODUCT_BF_E4, "window opcode 3 must stay the lean BFxE4 class");
...
constexpr u16 BWD_WINDOW_SHAPE_DEFINED_BITS = (1u << 11) - 1;

static_assert(BWD_WINDOW_SHAPE_DEFINED_BITS == 0x7ff, "window shape mask width drift");

// A source operand is `slot:6 << 7 | column:7`; the window stream carries it
// directly rather than through a slot table.
static_assert(BWD_SOURCE_LANE_COLUMN_BITS == 7, "window source packing drift");
...
static_assert(BWD_WINDOW_ROWS_PER_TILE == BWD_WINDOW_WARP_LANES, "a row tile is one warp of rows");
