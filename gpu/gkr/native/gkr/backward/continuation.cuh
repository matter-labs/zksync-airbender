...
// Maximum source count for the compact continuation descriptor's source array.
...
// Reads the bf value at `index` from a real source pointer or synthesizes it
// from a virtual kind.
...
// Folds the bf source pair at `(index, base_layer_half_size + index)` and
// writes the folded E value into the cache at `index` if `first_access`.
template <typename E, typename Desc>
...
// Fold-on-first-load for round 1 ext sources (compact-resolved).
template <typename E, typename Desc>
...
// Process a range of unified terms from cache for round 1 compact, loading via
// the compact decoder. Templated
...
// Compute path for the compact continuation descriptor, loading from
// compact-resolved cache.
