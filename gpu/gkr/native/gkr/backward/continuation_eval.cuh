...
DEVICE_FORCEINLINE void bwd_continuation_apply_immediate(const u32 *immediates, const u16 immediate_id, const e4 &value, e4 &sum) {
  if (immediate_id == BWD_SEG_IMMEDIATE_ONE) {
    ...
    const bf immediate = bf::from_reduced_raw_repr(immediates[immediate_id - BWD_SEG_IMMEDIATE_RESERVED]);
...
//
// so the VM can preserve its endpoint-only load while main-tail can resolve a
...
// index. Every coefficient access stays behind AB_GKR_BWD_SEG_COEFF.
template <typename SourcePairResolver>
DEVICE_FORCEINLINE void bwd_continuation_evaluate_list(const u16 *program, const u32 *immediates, u32 pc, const u32 pc_end, const bool apply_c_init,
  ...
  if (apply_c_init && has_c_init)
    acc_c0 = AB_GKR_BWD_SEG_COEFF(c_init_coeff_id);

#pragma unroll 1
  for (; pc < pc_end; pc += BWD_SEG_WORDS_PER_TERM) {
