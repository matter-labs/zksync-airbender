...
use gpu_ops::simple::{set_by_val, SetByRef, SetByVal};
...
pub(crate) fn const_derived_e4_source(r: &ChallengeRef) -> Result<ConstDerivedE4Source, BindError> {
    match (&r.key, &r.power) {
...
pub(crate) fn production_header<E>(
    ...
    assert!(
        inits_and_teardowns_top_bits.len() <= 16,
        "at most 16 init/teardown sets are supported"
    );
    let mut top_bits = [0; 16];
    top_bits[..inits_and_teardowns_top_bits.len()].copy_from_slice(inits_and_teardowns_top_bits);
    FwdVmHeaderInputs {
        mapping_arena: [
        ...
        inits_and_teardowns_top_bits: top_bits,
        num_inits_and_teardowns_top_bits: inits_and_teardowns_top_bits.len(),
    }
}
...
#[cfg(test)]
pub(crate) const AB_GKR_FWD_VM_POISON_DESTINATIONS_ENV: &str = "AB_GKR_FWD_VM_POISON_DESTINATIONS";
