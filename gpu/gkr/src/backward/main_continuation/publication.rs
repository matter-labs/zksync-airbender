...
#[allow(dead_code)] // Adoption-only variants become live when Task 6 wires the producer.
...
fn validate_adoption_shape(
    expected: ContinuationPublishedShape,
    actual: ContinuationPublishedShape,
) -> Result<(), ContinuationPublicationError> {
    if actual != expected {
        return Err(ContinuationPublicationError::ShapeMismatch { expected, actual });
    }
    Ok(())
}

pub(crate) fn validate_adoption_state(
    expected: Option<ContinuationPublishedShape>,
    actual: ContinuationPublishedShape,
    expected_depth_is_live: bool,
) -> Result<ContinuationPublishedShape, ContinuationPublicationError> {
    let expected = expected.ok_or(ContinuationPublicationError::UnexpectedPublishedLevel)?;
    validate_adoption_shape(expected, actual)?;
    if expected_depth_is_live {
        return Err(ContinuationPublicationError::AlreadyAdopted {
...
pub(crate) fn preserve_owned_on_validation_error<T, E>(
    owner: T,
    validate: impl FnOnce(&T) -> Result<(), E>,
) -> Result<T, (T, E)> {
    match validate(&owner) {
        Ok(()) => Ok(owner),
        Err(error) => Err((owner, error)),
    }
}

pub(super) fn source_byte_offset(
    shape: ContinuationPublishedShape,
    source: SourceId,
) -> Result<usize, ContinuationPublicationError> {
    shape_elems(shape)?;
    let source_index =
        usize::try_from(source.0).map_err(|_| ContinuationPublicationError::SourceOutOfBounds {
            source,
            columns: shape.columns,
        })?;
    if source_index >= shape.columns {
    ...
    source_index
        .checked_mul(shape.column_elems)
        .and_then(|offset| offset.checked_mul(size_of::<E4>()))
        .ok_or(ContinuationPublicationError::ShapeElementCountOverflow)
}
...
#[allow(dead_code)] // Task 5 constructs it; Task 6 transfers it between consumers.
pub(crate) struct ContinuationPublishedLevel {
    shape: ContinuationPublishedShape,
    allocation: DeviceAllocation<E4>,
}

#[allow(dead_code)] // The runtime producer and second consumer land in Tasks 5-6.
impl ContinuationPublishedLevel {
    ...
    pub(crate) fn source_ptr(&self, source: SourceId) -> *const E4 {
        let byte_offset = source_byte_offset(self.shape, source)
            .unwrap_or_else(|error| panic!("continuation publication source pointer: {error}"));
        self.as_ptr()
            .cast::<u8>()
    ...
    pub(crate) fn into_allocation(self) -> DeviceAllocation<E4> {
        self.allocation
    }
}
...
mod cpu_continuation_published_level {
    ...
    use super::{
        preserve_owned_on_validation_error, source_byte_offset, validate_adoption_shape,
        validate_adoption_state, validate_canonical_publication, ContinuationPublicationError,
        FinalEvaluationRepointError,
    };
    ...
    #[test]
    fn cpu_continuation_published_level_rejects_each_adoption_shape_mismatch() {
        ...
        for actual in [
            ...
            assert!(matches!(
                validate_adoption_shape(expected, actual),
                Err(ContinuationPublicationError::ShapeMismatch { .. })
            ));
        }
    }

    #[test]
    fn cpu_continuation_published_level_returns_owner_on_every_adoption_rejection() {
        let expected = ContinuationPublishedShape {
            depth: 12,
            columns: 3,
            column_elems: 16,
        ...
        for (mutation, (expected_shape, actual_shape, occupied)) in
            mutations.into_iter().enumerate()
        {
            let owner = vec![0xa5, mutation as u8, 0x5a];
            let (returned, actual) = preserve_owned_on_validation_error(owner, |_| {
                validate_adoption_state(expected_shape, actual_shape, occupied).map(|_| ())
            })
            .unwrap_err();
            assert_eq!(returned, vec![0xa5, mutation as u8, 0x5a]);
            assert!(matches!(
                (mutation, actual),
                (0, ContinuationPublicationError::UnexpectedPublishedLevel)
                    | (1, ContinuationPublicationError::ShapeMismatch { .. })
                    | (2, ContinuationPublicationError::AlreadyAdopted { .. })
            ));
        }
        let owner = vec![0xa5, 0xff, 0x5a];
        assert_eq!(
            preserve_owned_on_validation_error(owner, |_| {
                validate_adoption_state(Some(expected), expected, false).map(|_| ())
            })
            .unwrap(),
            vec![0xa5, 0xff, 0x5a]
        );
    ...
    #[test]
    fn cpu_continuation_published_level_offsets_are_column_major_and_checked() {
        let shape = ContinuationPublishedShape {
            depth: 15,
            columns: 4,
            column_elems: 8,
        };
        assert_eq!(source_byte_offset(shape, SourceId(0)), Ok(0));
        assert_eq!(
            source_byte_offset(shape, SourceId(3)),
            Ok(3 * 8 * size_of::<gpu_core::primitives::field::E4>())
        );
        assert!(matches!(
            source_byte_offset(shape, SourceId(4)),
            Err(ContinuationPublicationError::SourceOutOfBounds { .. })
        ));
    }
