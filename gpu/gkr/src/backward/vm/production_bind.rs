...
//! clamp is shared.
//!
//! [`resolve_storage_column`]: crate::forward::vm::production_bind::resolve_storage_column
...
pub(crate) fn schedule_bwd_vm_round0(
    ...
    if poison_accumulator_enabled() {
        ...
        gpu_ops::simple::set_by_val(E4::from_array_of_base([BF::new(POISON); 4]), dst, stream)?;
    }
    #[cfg(not(test))]
    let _ = contributions;
...
///
/// [`fold_factored_eq_one_round`]: crate::backward::kernels::fold_factored_eq_one_round
...
pub(crate) fn schedule_fold_storage_poison<E: Copy>(
    ...
    let mut fill = |arc: &std::sync::Arc<DeviceAllocation<E>>| -> CudaResult<()> {
        ...
        gpu_ops::simple::set_by_val(value, dst, stream)
    };
