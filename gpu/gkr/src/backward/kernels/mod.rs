...
pub use shared::{
    make_deferred_backward_workflow_state, ClaimBufferLayout, DeviceClaimPointAndBatching,
    GpuGKRBackwardScheduledExecution, ScheduledBackwardWorkflowState,
    ScheduledBackwardWorkflowStateHandle,
};
...
/// the combined `crate::GpuKernels` supertrait aggregates this with
...
impl BackwardKernels for gpu_core::primitives::field::E4 {
    // --- backward (dim-reducing) ------------------------------------------
    const PAIRWISE_ROUND0: GpuDimensionReducingPairwiseRound0Signature<Self> =
...
#[cfg(test)]
pub(crate) use tests::apply_eq_and_reduce_accumulator;

// `pub` re-exports of the relocated backward-workflow test-driver helpers +
// `GpuGKRBackwardExecution` (moved to `shared` out of `#[cfg(test)] mod tests`):
// the apex e2e suite drives the backward workflow through these. Test-reference
// surface, `#[doc(hidden)]` at their definitions.
pub use shared::{
    h2d_claim_point_and_batching_from_host, h2d_claims_from_host,
