...
use crate::upstream::{
    ...
    MerkleTreeCapVarLength, OutputType, SumcheckIntermediateProofValues, SumcheckRoundCoefficients,
...
/// slab. POD captured by the forward pass / stage-1 trace holders; the
/// gkr-side builder converts these into [`ProofLayoutBaseLayerGeometry`].
#[derive(Clone, Copy, Debug)]
pub struct GpuGKRTraceGeometry {
...
#[derive(Debug, Clone)]
pub struct WhirIntermediateDims {
    pub cap_digest_count: usize,
    pub query_count: usize,
...
pub struct WhirDims {
    ...
    pub intermediate: Vec<WhirIntermediateDims>,
...
pub(crate) struct OutputEvaluationsLayout {
    pub read_set: Range<usize>,
    pub write_set: Range<usize>,
}
...
pub(crate) struct BackwardLayerLayout {
    ...
    pub internal_round_coefficients: Range<usize>,
    ...
    pub final_step_evaluations: Range<usize>,
...
pub(crate) struct WhirLayout {
    ...
    pub base_query_indices: Range<usize>,
    pub setup: WhirBaseLayerByteLayout,
    pub memory: WhirBaseLayerByteLayout,
    pub witness: WhirBaseLayerByteLayout,
    pub intermediate: Vec<WhirIntermediateByteLayout>,
    /// `ood_samples` — `num_ood_samples` `E4` values.
    pub ood_samples: Range<usize>,
    /// `sumcheck_polys` — flat array of `[E4; 3]`, length `total_sumcheck_polys`.
    pub sumcheck_polys: Range<usize>,
    /// `pow_nonces` — `pow_rounds` `u64` values.
    pub pow_nonces: Range<usize>,
    /// `final_monomials` — `final_monomials_len` `E4` values.
    pub final_monomials: Range<usize>,
}
...
impl ProofLayoutBaseLayerGeometry {
    pub fn from_geometry(geometry: GpuGKRTraceGeometry, columns_count: usize) -> Self {
        Self {
            columns_count,
...
impl ProofLayout {
    pub fn new(inputs: &ProofLayoutInputs) -> Self {
        let mut cur = 0usize;
