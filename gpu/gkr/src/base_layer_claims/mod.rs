...
use gpu_core::primitives::context::{DeviceAllocation, UnsafeAccessor, UnsafeMutAccessor};
...
pub struct ScheduledBaseLayerClaimsState<E> {
...
#[allow(dead_code)]
pub struct GpuGKRBaseLayerClaimsScheduledExecution<E> {
...
impl<E: Copy + 'static> GpuGKRBaseLayerClaimsScheduledExecution<E> {
    pub fn shared_state_handle(&mut self) -> UnsafeMutAccessor<ScheduledBaseLayerClaimsState<E>> {
    ...
    /// own the closure.
    pub fn take_pending_aggregation(&mut self) -> Box<dyn Fn() + Send + Sync + 'static> {
...
where
    E: BackwardKernels + Field + gpu_cub::cub::device_reduce::Reduce + 'static,
{
...
pub fn schedule_prepare_base_layer_claims_with_sources<E>(
        ...
        + 'static,
{
    ...
    if !extras_plan.sources.is_empty() {
        ...
        gpu_hash::blake2s::gather_e_addresses(&src_ptrs, d_values_e4, stream)?;
