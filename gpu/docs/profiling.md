...
| build-diag rebuild | rebuild with the crate's `GPU_<X>_ENABLE_BUILD_DIAG=1` — `gpu_native_build` wires it to `nvcc --ptxas-options=-v` (per-kernel register/spill/smem report; captured to `target/<profile>/build/<crate>-<hash>/stderr`, echoed only under `cargo build -vv`) plus `--keep` (PTX/cubin intermediates retained in the crate's CMake build dir) |
