    ...
    ColumnMajorExtensionOracleForCoset<F, E>
{
    pub fn values_for_folded_index(&self, index: usize, values_per_leaf: usize) -> Vec<E> {
        let trace_len = self.values_normal_order.column.len();
        assert!(values_per_leaf.is_power_of_two());
        assert!(
...
/// Opaque intermediate oracle: a query function (same tuple as
/// `ColumnMajorExtensionOracleForLDE::query_for_folded_index`) plus the coset count
/// (needed to size the next round's query domain). Depending on the mode the closure
/// owns either the whole extension oracle or a coset-by-coset commitment.
#[expect(
    clippy::type_complexity,
    reason = "generic over field + allocator; a bound-free type alias would drop those bounds"
)]
struct IntermediateOracle<'a, F, E, T>
where
    F: PrimeField + TwoAdicField + 'a,
    E: FieldExtension<F> + Field + 'a,
    T: ColumnMajorMerkleTreeConstructor<F> + 'a,
{
    num_cosets: usize,
    query: Box<dyn Fn(usize) -> (usize, Vec<E>, ExtensionFieldQuery<F, E, T>) + 'a>,
}

/// Build the intermediate oracle for a folded monomial form, returning its Merkle
/// cap (to commit) and the opaque query handle.
fn build_intermediate_oracle<'a, F, E, T>(
    monomial_form: &[E],
    lde_factor: usize,
    values_per_leaf: usize,
    tree_cap_size: usize,
    mode: WhirIntermediateOracleMode,
    twiddles: &'a Twiddles<F, Global>,
    worker: &'a Worker,
) -> (MerkleTreeCapVarLength, IntermediateOracle<'a, F, E, T>)
where
    F: PrimeField + TwoAdicField + 'a,
    E: FieldExtension<F> + Field + 'a,
    T: ColumnMajorMerkleTreeConstructor<F> + 'a,
    [(); E::DEGREE]: Sized,
{
    match mode {
        WhirIntermediateOracleMode::Monolithic => {
            ...
            let num_cosets = oracle.cosets.len();
            let query = Box::new(move |qi: usize| oracle.query_for_folded_index(qi));
            (cap, IntermediateOracle { num_cosets, query })
        }
        WhirIntermediateOracleMode::CosetByCoset => {
            ...
            let query = Box::new(move |qi: usize| commitment.query(qi, twiddles, worker));
            (
                cap,
                IntermediateOracle {
    ...
    [(); E::DEGREE]: Sized,
{
    ...
    // initial round where we fold and query existing oracles
    {
        ...
        for &query_index in query_indexes.iter() {
                ...
                oracle_refs.iter().zip(batch_challenges.iter()).enumerate()
            {
                ...
                if !oracle.cosets[0].original_values_normal_order.is_empty() {
    ...
    for internal_round in 0..num_internal_whir_steps {
        ...
        for &query_index in query_indexes.iter() {
            ...
            let (_coset_idx, coeffs, query) = (rs_oracle_to_query.query)(query_index);
    ...
    // check consistency between them, and perform final explicit sumcheck
    {
        ...
        for _folding_round in 0..num_folding_steps {
            ...
            let next_claim = evaluate_small_univariate_poly(&univariate_coeffs, &folding_challenge);
            // In this final round `claim` is only read again by the `gkr_self_checks` asserts
            // (next iteration's round check and the post-loop full-sum check), so with those
            // compiled out this last store is dead. Kept so both builds share the same codegen.
            #[cfg_attr(not(feature = "gkr_self_checks"), expect(unused_assignments))]
            {
                claim = next_claim;
            }
            // and fold the poly itself - both multivariate evals mapping, and monomial form
...
mod test {
    ...
    fn test_domain_hypercube_evals() {
        let _worker = Worker::new_with_num_threads(1);
        let size: usize = 4;
