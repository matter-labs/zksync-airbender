...
pub trait ExtensionOnlyRoundImplementation<F: PrimeField, E: FieldExtension<F> + Field>:
    'static + Send + Sync
{
...
pub fn evaluate_extension_only_rounds_with_full_sized_scratch_parallel<
    F: PrimeField,
    E: FieldExtension<F> + Field,
    I: ExtensionOnlyRoundImplementation<F, E>,
>(
    base_field_folding_buffers: Vec<DisjointAccessQuasiSlice<E, false>>,
    ext_field_folding_buffers: Vec<DisjointAccessQuasiSlice<E, false>>,
    ...
    worker.scope_with_threshold(work_size, PAR_THRESHOLD, |scope, geometry| {
        ...
        for thread_idx in 0..geometry.num_chunks {
            ...
            let mut base_field_folding_buffers = base_field_folding_buffers.clone();
            let mut ext_field_folding_buffers = ext_field_folding_buffers.clone();
            let acc_dst = it.next().expect("dst chunk");

            Worker::smart_spawn(scope, thread_idx == geometry.len() - 1, move |_| {
                *acc_dst = evaluate_extension_only_rounds_with_full_sized_scratch::<F, E, I>(
                    &mut base_field_folding_buffers,
                    &mut ext_field_folding_buffers,
                    description,
...
pub fn evaluate_extension_only_rounds_with_full_sized_scratch<
    F: PrimeField,
    E: FieldExtension<F> + Field,
    I: ExtensionOnlyRoundImplementation<F, E>,
>(
    base_field_folding_buffers: &mut [DisjointAccessQuasiSlice<E, false>],
    ext_field_folding_buffers: &mut [DisjointAccessQuasiSlice<E, false>],
    description: &BatchEvaluationCompactDescription<F, E>,
    precomputed_eq_prefix: &I::FoldingPrefix,
    precomputed_eq_suffix: &[E],
    unfolded_input_size_log2: usize,
    ...
    assert_eq!(
        ...
        base_field_folding_buffers.len()
    );
    assert_eq!(
        description.ext_read_with_interpolation.len(),
        ext_field_folding_buffers.len()
    );

    let unfolded_input_size = 1 << unfolded_input_size_log2;
    ...
    for row in row_range {
        ...
        for ((dst, interpolate_at_inf), buffer) in field_scratch[..base_field_folding_buffers.len()]
            .iter_mut()
            .zip(description.base_read_with_interpolation.iter())
            .zip(base_field_folding_buffers.iter_mut())
        {
            if *interpolate_at_inf {
                I::read_then_fold_and_interpolate(
                    dst,
                    buffer,
                    precomputed_eq_prefix,
                    unfolded_input_size,
                ...
                I::read_then_fold_without_interpolation(
                    dst,
                    buffer,
                    precomputed_eq_prefix,
                    unfolded_input_size,
        ...
        for ((dst, interpolate_at_inf), buffer) in field_scratch[base_field_folding_buffers.len()..]
            ...
            .zip(ext_field_folding_buffers.iter_mut())
        {
            if *interpolate_at_inf {
                I::read_then_fold_and_interpolate(
                    dst,
