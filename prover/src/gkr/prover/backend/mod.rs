...
mod tests {
    ...
    /// commitment that stores the codeword in BITREVERSED order can skip it:
    /// feed the scaled coefficients DIRECTLY (natural order) into the
    /// natural->bitreversed CT kernel with the SAME bitreversed twiddle table.
    /// Pins the ordering fact: per coset, the alternative equals the naive
    /// codeword up to the in-coset bitreversal permutation, nothing else.
    #[test]
    fn no_bitreverse_lde_matches_naive_up_to_in_coset_bitreversal() {
        ...
        let twiddles = Twiddles::<BabyBearField, Global>::new(n, &worker);
        ...
        for (coset_idx, offset) in offsets.iter().enumerate() {
            ...
            fft::column_major::naive::serial_ct_ntt_natural_to_bitreversed::<
                BabyBearField,
                BabyBearField,
            >(&mut alt[..], log_n, selected_twiddles);
