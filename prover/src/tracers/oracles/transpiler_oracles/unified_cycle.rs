...
use crate::tracers::main_cycle_optimized::SingleCycleTracingData;
...
pub struct UnifiedCycleOracle<'a> {
    pub cycle_data: &'a [SingleCycleTracingData],
    // NOTE: for the rows when we actually do not execute anything
    pub padding_case: SingleCycleTracingData,
}

impl<'a, F: PrimeField> Oracle<F> for UnifiedCycleOracle<'a> {
    #[track_caller]
    ...
    fn get_boolean_witness_from_placeholder(
        ...
        // special case if we ever execute
        if placeholder == Placeholder::ExecuteOpcodeFamilyCycle {
            if trace_step < self.cycle_data.len() {
                return true;
            } else {
                return false;
            }
