...
pub fn create_blake_g_function_control_and_offsets_table<F: PrimeField>(id: u32) -> LookupTable<F> {
    let mut keys = Vec::with_capacity(1 << (BLAKE2S_G_FUNCTION_NUM_CONTROL_REGISTER_BITS + 1));
    for control_with_exe in 0..1 << (BLAKE2S_G_FUNCTION_NUM_CONTROL_REGISTER_BITS + 1) {
    ...
    LookupTable::create_table_from_key_and_pure_generation_fn(
        ...
        |keys| {
            ...
            debug_assert!(input < (1 << (BLAKE2S_G_FUNCTION_NUM_CONTROL_REGISTER_BITS + 1)));
            let execute = (input >> BLAKE2S_G_FUNCTION_NUM_CONTROL_REGISTER_BITS) > 0;
            let x12 = input & ((1 << BLAKE2S_G_FUNCTION_NUM_CONTROL_REGISTER_BITS) - 1);
            ...
            if execute {
                ...
                for _ in
                    0..(1 + BLAKE2S_G_FUNCTION_X10_NUM_WRITES + BLAKE2S_G_FUNCTION_X11_NUM_READS)
                {
                    result.push(F::ZERO);
                }
            }
