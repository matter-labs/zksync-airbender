...
pub(super) fn apply_unified_mem_word_only_lw_sw_data_path<F: PrimeField, CS: Circuit<F>>(
    ...
    // `is_fam4_sum()` == `is_lw + is_sw`, inlined here as an Expr sum (the
    // Constraint-typed closure is retained for the out-of-scope lookup block).
    cs.add_constraint_expr(
        Expr::from(gate_fam4_rom)
            - (Expr::from(is_lw) + Expr::from(is_sw)) * Expr::from(is_rom_base_layer),
    );
    {
        let value_fn = move |placer: &mut CS::WitnessPlacer| {
            let is_lw_raw = placer.get_boolean(is_lw_var);
