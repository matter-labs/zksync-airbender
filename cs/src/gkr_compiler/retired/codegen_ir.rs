...
mod tests {
    use super::*;
    // These fixtures were relocated into the shared `test_support` module so they
    // can be reused cross-crate. Inside this `mod tests`, `super::` resolves to
    // `codegen_ir`, NOT `gkr_compiler`, so the full crate path is required.
    use crate::gkr_compiler::test_support::{
        build_add_sub_artifact, build_add_sub_artifact_no_caches, metadata_fixtures,
        single_gate_layer,
    };
