use super::load_store::MEMORY_GET_OFFSET_AND_MASK_NUM_BITS_NO_TRAP;
use super::*;

pub fn subword_only_load_store_tables() -> Vec<TableType> {
    vec![
...
pub fn subword_only_load_store_table_addition_fn<F: PrimeField, CS: Circuit<F>>(cs: &mut CS) {
    for el in subword_only_load_store_tables() {
        cs.materialize_table(el);
    }
}

pub fn subword_only_load_store_table_driver_fn<F: PrimeField>(table_driver: &mut TableDriver<F>) {
    for el in subword_only_load_store_tables() {
...
>(
    ...
    let [rom_load_low, rom_load_high] = {
        ...
        [rom_less_than_word_load_low, rom_less_than_word_load_high]
    };
...
mod test {
    ...
    #[test]
    fn compile_subword_only_load_store_circuit() {
        ...
        let compiled = compile_unrolled_circuit_state_transition::<Mersenne31Field>(
            ...
            &|cs| {
                subword_only_load_store_circuit_with_preprocessed_bytecode::<_, _, SECOND_WORD_BITS>(
                    cs,
                )
            },
        ...
        serialize_to_file(
            &compiled,
            "subword_only_load_store_preprocessed_layout.json",
        );
    }
    ...
    fn compile_subword_only_load_store_witness_graph() {
        ...
        serialize_to_file(&ssa_forms, "subword_only_load_store_preprocessed_ssa.json");
    }
}
