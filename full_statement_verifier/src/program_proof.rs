...
use common_constants::{BIGINT_OPS_WITH_CONTROL_CSR_REGISTER, BLAKE2S_DELEGATION_CSR_REGISTER, BLAKE2S_G_FUNCTION_DELEGATION_CSR_REGISTER, KECCAK_SPECIAL5_CSR_REGISTER};
...
pub struct ProgramProof {
    pub riscv_proofs: BTreeMap<u32, Vec<GKRProof<BabyBearField, BabyBearExt4, DefaultTreeConstructor>>>,
    pub compiled_riscv_circuits: BTreeMap<u32, GKRCircuitArtifact<BabyBearField>>,
    pub inits_and_teardown_proofs: Vec<GKRProof<BabyBearField, BabyBearExt4, DefaultTreeConstructor>>,
    pub inits_and_teardowns_circuit: Option<GKRCircuitArtifact<BabyBearField>>,
    pub delegation_proofs: BTreeMap<u32, Vec<GKRProof<BabyBearField, BabyBearExt4, DefaultTreeConstructor>>>,
...
impl ProgramProof {
    ...
    pub fn flatten_for_verification(
        &self,
    ) -> Vec<u32> {
        ...
        for (k, proofs) in self.riscv_proofs.iter() {
            ...
            for proof in proofs.iter() {
                responses.extend(::verifier_common::gkr::flatten::flatten_gkr_proof_for_nds(proof, compiled_circuit));
            }
        }

        if self.inits_and_teardown_proofs.len() > 0 {
            responses.push(1u32);
            let compiled_circuit = &self.inits_and_teardowns_circuit.as_ref().expect("compiled inits and teardowns");
