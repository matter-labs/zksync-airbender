...
struct TracerRanges<T: Copy + 'static, A: HostTraceAllocator> {
    queue: VecDeque<PtrRange<T, A>>,
    current: PtrRange<T, A>,
    count: usize,
}

impl<T: Copy + 'static, A: HostTraceAllocator> TracerRanges<T, A> {
    fn new(queue: VecDeque<PtrRange<T, A>>) -> Self {
        Self {
            queue,
...
pub(crate) trait Tracer<A: HostTraceAllocator>: WitnessTracer {
    type Ranges: DataTraceRanges + Send;

    fn new(trace_ranges: Self::Ranges) -> Self;
}

pub(crate) struct SplitTracer<A: HostTraceAllocator> {
    ...
    slt_branch_family: TracerRanges<NonMemoryOpcodeTracingDataWithTimestamp, A>,
    mul_div_family: TracerRanges<NonMemoryOpcodeTracingDataWithTimestamp, A>,
    word_size_mem_family: TracerRanges<MemoryOpcodeTracingDataWithTimestamp, A>,
    subword_size_mem_family: TracerRanges<MemoryOpcodeTracingDataWithTimestamp, A>,
}

impl<A: HostTraceAllocator> Tracer<A> for SplitTracer<A> {
...
impl<A: HostTraceAllocator> WitnessTracer for SplitTracer<A> {
    #[inline(always)]
...
pub(crate) struct UnifiedTracer<A: HostTraceAllocator> {
    blake_calls: TracerRanges<Blake2sRoundFunctionDelegationWitness, A>,
    bigint_calls: TracerRanges<BigintDelegationWitness, A>,
    keccak_calls: TracerRanges<KeccakSpecial5DelegationWitness, A>,
    keccak_column_parity_calls: TracerRanges<KeccakColumnParityDelegationWitness, A>,
...
impl<A: HostTraceAllocator> Tracer<A> for UnifiedTracer<A> {
...
impl<A: HostTraceAllocator> WitnessTracer for UnifiedTracer<A> {
    #[inline(always)]
    fn needs_tracing_data_for_circuit_family<const FAMILY: u8>(&self) -> bool {
        true
    }
